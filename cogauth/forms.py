...
class COGAuthenticationForm(forms.Form):
    username = forms.RegexField(
        ...
        error_messages={
            "invalid": _(
                "Username is typically an email address. It may not contain certain special characters (such as spaces).  It is CASE SENSITIVE."
            )
        },
    )
...
class COGPasswordRegisterForm(forms.Form):
    ...
    class Meta:
        widgets = {"countrycode": CountrySelectWidget()}


class COGResetMFA(forms.Form):

    reason = forms.CharField(widget=forms.Textarea(), label=_("Reason for MFA reset"))
...
class COGVerifyEmailForm(forms.Form):
    code = forms.CharField(
        max_length=10, required=True, widget=forms.TextInput(attrs={"autofocus": "autofocus", "autocomplete": "off"})
    )
...
class COGResetPasswordForm(forms.Form):
    code = forms.CharField(max_length=10, required=True, widget=forms.TextInput(attrs={"autocomplete": "off"}))

    new_password1 = forms.CharField(max_length=50, required=True, widget=forms.PasswordInput, label="New Password")

    new_password2 = forms.CharField(
        max_length=50, required=True, widget=forms.PasswordInput, label="Please re-enter Password"
    )
...
class COGPasswordChangeForm(forms.Form):
    old_password = forms.CharField(max_length=50, required=True, widget=forms.PasswordInput, label="Current Password")
...
class ProfileForm(forms.Form):
    ...
    email = forms.EmailField(required=True, disabled=True, label="Login Username/Email. This field is case sensitive.")
    ...
    org = forms.CharField(max_length=200, label="Company/Affiliation", required=False)

    title = forms.CharField(
        max_length=200, label="Job Title", help_text=_("This field is visible to other VINCE users"), required=False
    )

    timezone = forms.ChoiceField(label="Timezone", choices=[(x, x) for x in pytz.common_timezones])
...
class SignUpForm(UserCreationForm):
    ...
    preferred_username = forms.RegexField(
        ...
        max_length=254,
        help_text=_(
            "The name visible to other VINCE users. It may only contain 1 space and may not contain certain special characters. (You can modify this later)"
        ),
        regex=r"^[-\w\+]+(\s[-\w\+]+)*$",
        required=True,
        error_messages={
