...
urlpatterns = [
    ...
    re_path(
        r"^user/admin/contacts/reports/(?P<type>[1-5])/$",
        views.VinceContactReportsView.as_view(),
        name="contactreports",
    ),
    # path('user/admin/notifications', views.ManageAlertsView.as_view(), name='managenotifications'),
    ...
    re_path(
        r"^casevendor/confirm/edit/date/(?P<pk>[0-9]+)/$",
        views.ConfirmVendorNotifyDate.as_view(),
        name="confirmvendordate",
    ),
    ...
    re_path(r"^case/(?P<pk>[0-9]+)/participants/$", views.CaseParticipantsView.as_view(), name="caseparticipantstab"),
    re_path(r"^casevendor/(?P<pk>[0-9]+)/$", views.CaseVendors.as_view(), name="casevendors"),
    re_path(
        r"^casevendor/(?P<pk>[0-9]+)/(?P<notify>notify)/$",
        views.NotifyVendorsListView.as_view(),
        name="casevendornotify",
    ),
    re_path(r"^casevendor/(?P<pk>[0-9]+)/status/$", views.VendorStatusView.as_view(), name="vendorstatus"),
    re_path(
        r"^casevendor/(?P<pk>[0-9]+)/status/changes/$",
        views.VendorStatusChangesView.as_view(),
        name="vendorstmtchanges",
    ),
    re_path(
        r"^casevendor/(?P<pk>[0-9]+)/status/edit/$", views.EditVendorStatusView.as_view(), name="editvendorstatus"
    ),
    re_path(
        r"^casevendor/(?P<vendor_id>[0-9]+)/status/edit/(?P<pk>[0-9]+)/$",
        views.VendorVulStatement.as_view(),
        name="getvulstmt",
    ),
    re_path(
        r"^casevendor/(?P<pk>[0-9]+)/modal/status/$", views.VendorStatusModalView.as_view(), name="vendorstatusmodal"
    ),
    ...
    re_path(
        r"^case/(?P<pk>[0-9]+)/activity/results/$", views.CommunicationsFilterResults.as_view(), name="commfilter"
    ),
    ...
    re_path(
        "contact/remove/product/(?P<pk>[0-9]+)/(?P<product>[0-9]+)/$",
        views.RemoveProductFromContact.as_view(),
        name="rmproduct",
    ),
    ...
    path(
        "contact/verify/complete/list/",
        views.CompletedContactAssociationListView.as_view(),
        name="complete_contact_list",
    ),
    re_path(r"^contact/verify/request/(?P<pk>[0-9]+)/$", views.ContactRequestAuth.as_view(), name="contactreqauth"),
    re_path(
        r"^contact/verify/request/complete/(?P<pk>[0-9]+)/$",
        views.CompleteContactAssociation.as_view(),
        name="complete_contact",
    ),
    re_path(
        r"^contact/verify/request/restart/(?P<pk>[0-9]+)/$",
        views.RestartContactAssociation.as_view(),
        name="restart_contact",
    ),
    ...
    re_path(r"reports/vulnotes/(?P<pk>[0-9]+)/$", views.ReportsVulnotesView.as_view(), name="reports_page_vulnotes"),
    ...
    re_path(
        ...
        name="reports_page_current_ticket_status",
    ),
    re_path(r"reports/cves/(?P<pk>[0-9]+)/$", views.ReportsCVEsView.as_view(), name="reports_page_cves"),
    re_path(r"reports/tags/(?P<pk>[0-9]+)/$", views.ReportsTagsView.as_view(), name="reports_page_tags"),
    ...
    re_path(
        "reports/print/(?P<month>[0-9]+)/(?P<year>[0-9]+)/$", views.PrintReportsView.as_view(), name="printreport"
    ),
    re_path(
        "reports/(?P<pk>[0-9]+)/print/(?P<month>[0-9]+)/(?P<year>[0-9]+)/$",
        views.PrintReportsView.as_view(),
        name="printreport",
    ),
    re_path(
        "reports/weekly/(?P<pk>[0-9]+)/print/(?P<week>[0-9]+)/(?P<year>[0-9]+)/$",
    ...
    re_path(
        "contact/vcuser/(?P<pk>[0-9]+)/bounces/$", views.VinceCommUserBouncesView.as_view(), name="vcuserbouncestab"
    ),
    ...
    re_path(
        "contact/edit/email/modify/(?P<pk>[0-9]+)/$", views.ChangeEmailNotifications.as_view(), name="changeemail"
    ),
    re_path(
        "contact/remove/email/(?P<pk>[0-9]+)/(?P<email>[0-9]+)/$",
        views.RemoveEmailFromContact.as_view(),
        name="rmemail",
    ),
    ...
    re_path(
        "^removevendor/confirm/(?P<vendor>[0-9]+)/$",
        views.ConfirmRemoveVendorFromCase.as_view(),
        name="rmvendorconfirm",
    ),
    re_path(
        "^removeallvendor/confirm/(?P<pk>[0-9]+)/$",
        views.ConfirmRemoveAllVendorsFromCase.as_view(),
        name="rmallvendors",
    ),
    ...
    re_path("^taguser/$", views.TagUser.as_view(), name="taguser"),
    ...
    re_path(
        "^ticket/(?P<pk>[0-9]+)/artifact/(?P<artifact>[0-9]+)/$",
        views.RemoveTicketArtifact.as_view(),
        name="rmartifact",
    ),
    ...
    re_path("^case/(?P<pk>[0-9]+)/timeline/$", views.CaseTimelineView.as_view(), name="case_timeline"),
    ...
    re_path("^case/(?P<pk>[0-9]+)/posts/$", views.CasePostsView.as_view(), name="casepoststab"),
    re_path("^case/(?P<pk>[0-9]+)/vulnote/$", views.CaseVulNoteView.as_view(), name="casevulnotetab"),
    re_path(
        "^case/(?P<pk>[0-9]+)/noticketorcr/$", views.CaseNoTicketOrReportView.as_view(), name="casenoticketorreporttab"
    ),
    re_path("^case/(?P<pk>[0-9]+)/cr/$", views.CaseOriginalReportView.as_view(), name="caseoriginalreporttab"),
    re_path("^case/(?P<pk>[0-9]+)/vulnerabilities/$", views.CaseVulsView.as_view(), name="casevulstab"),
    ...
    re_path(
        "^tickets/dashboard/(?P<type>(Open|Progress|Message))?/$",
        views.DashboardTicketView.as_view(),
        name="dashboardtickets",
    ),
    re_path("^dash/case/(?P<pk>[0-9]+)/activity/$", views.DashboardCaseActivityView.as_view(), name="dashactivity"),
    re_path(
        "^dash/queue/(?P<title>(Open|Progress|Message))/activity/$",
        views.DashboardQueueActivityView.as_view(),
        name="dashqueue",
    ),
    ...
    re_path(
        "^case/(?P<pk>[0-9]+)/notify/crview/(?P<notify_id>[0-9]+)/$",
        views.WritePostCRView.as_view(),
        name="editpost_crview",
    ),
    ...
    re_path(
        "^case/(?P<pk>[0-9]+)/ticket/artifact/share/$", views.MakeTicketArtifactPublic.as_view(), name="maketktpublic"
    ),
    ...
    re_path(
        ...
        views.DeleteTicketDependency.as_view(),
        name="rmdep",
    ),
    re_path("^add_case_dependency/(?P<case_id>[0-9]+)/$", views.AddCaseDependency.as_view(), name="addcasedep"),
    re_path(
        "^rm_case_dependency/(?P<case_id>[0-9]+)/(?P<dep_id>[0-9]+)/$",
        views.DeleteCaseDependency.as_view(),
        name="rmcasedep",
    ),
    ...
    re_path(
        "^tickets/(?P<ticket_id>[0-9]+)/attachment_delete/(?P<attachment_id>[0-9]+)/$",
        views.DeleteAttachment.as_view(),
        name="attachment_del",
    ),
    ...
    re_path(
        "^artifact/(?P<post>post)/share/(?P<pk>[0-9]+)/$",
        views.ShareArtifactView.as_view(),
        name="post_artifact_share",
    ),
    re_path("^vulnote/diff/(?P<revision_id>[0-9]+)/$", views.VulNoteDiffView.as_view(), name="diff"),
    re_path(
        "^vulnote/preview/(?P<pk>[0-9]+)/(?P<revision_id>[0-9]+)/$",
        views.VulNoteRevisionPreview.as_view(),
        name="preview",
    ),
    re_path(
        "^vulnote/change_revision/(?P<pk>[0-9]+)/(?P<revision_id>[0-9]+)/$",
        views.VulNoteChangeRevision.as_view(),
    ...
    re_path(
        "^email/create/admins/(?P<admins>admins)/(?P<pk>[0-9]+)/$",
        views.CreateNewEmailView.as_view(),
        name="newemailadmins",
    ),
    ...
    re_path(
        "^manage/cve/delete/(?P<pk>[0-9]+)/", views.CVEServicesDeleteAccount.as_view(), name="cve_services_delete"
    ),
...
try:
    if (settings.MULTIURL_CONFIG or settings.VINCE_NAMESPACE == "vince") and not (
        hasattr(settings, "LOCALSTACK") and settings.LOCALSTACK
    ):
        urlpatterns.extend(
            [
                path(
                    "login/", cogauth_views.COGLoginView.as_view(template_name="vince/tracklogin.html"), name="login"
                ),
        ...
        urlpatterns.extend(
            [
                path(
                    "login/",
                    auth_views.LoginView.as_view(extra_context={"token_login": 1}, template_name="vince/login.html"),
                    name="login",
                ),
