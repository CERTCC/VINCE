...
from vince.lib import (
    process_attachments,
    simple_merge,
    add_participant_vinny_case,
    add_coordinator_case,
    send_worker_email_all,
)
...
class TeamSettingsForm(forms.Form):
    outgoing_email = forms.EmailField(
        ...
        help_text=_(
            "This is the email address that will be used to send official case notifications "
            "to VINCE users. Editing this field requires superuser permissions."
        ),
    )

    vulnote_template = forms.CharField(
        widget=forms.Textarea(), label=_("Vulnerability Note Template"), initial=VULNOTE_TEMPLATE, required=False
    )

    team_signature = forms.CharField(widget=forms.Textarea(), label=_("Email Signature"), required=False)
    ...
    cna_email = forms.EmailField(
        ...
        help_text=(
            "If not set, this will default to the team email field above, or the default VINCE email if team email is not set."
        ),
    ...
    weekly_report_boolean = forms.BooleanField(
        ...
        help_text=_(
            "Ticked if information about this team should be sent to the designated email addresses on a weekly basis."
        ),
    )
...
class PreferencesForm(forms.Form):
    ...
    new_tickets = forms.MultipleChoiceField(
        choices=[], required=False, widget=forms.CheckboxSelectMultiple(attrs={"class": "ul_nobullet"})
    )
    ...
    email_on_ticket_change = forms.BooleanField(label=_("E-mail me on ticket change?"), required=False)

    email_on_ticket_assign = forms.BooleanField(label=_("E-mail me when assigned a ticket?"), required=False)
    ...
    reminder_tickets = forms.BooleanField(label=_("Remind me about tickets open > 14 days?"), required=False)

    reminder_publication = forms.BooleanField(label=_("Remind me about case expected publish dates?"), required=False)

    reminder_vendor_views = forms.BooleanField(
        label=_("Remind me about vendors that have not viewed a case?"), required=False
    )

    reminder_vendor_status = forms.BooleanField(
        label=_("Remind me about vendors that have not submitted status?"), required=False
    )

    reminder_cases = forms.BooleanField(
        label=_("Remind me about active cases that have not been modified > 14 days?"), required=False
    )
...
class CreateVulNote(forms.Form):
    content = forms.CharField(widget=forms.Textarea(), label=_("Contents"), required=False, initial=VULNOTE_TEMPLATE)
...
class EditVulNote(forms.Form):
    ...
    def clean(self):
        ...
        if not str(self.initial_revision.id) == str(self.presumed_revision):
            raise forms.ValidationError(gettext("While you were editing, someone else changed the revision."))
        if (
            ("title" in cd)
            and cd["title"] == self.initial_revision.title
            and cd["content"] == self.initial_revision.content
            and cd["references"] == self.initial_revision.references
        ):
...
class NotificationForm(forms.Form):
    ...
    email_template = forms.ChoiceField(
        choices=(), label="Choose an email template to send to the participant(s)", required=False
    )
    ...
    email_body = forms.CharField(
        ...
        help_text=_(
            "This email template (participant_notify_newcase) already contains a greeting, a signature, and a link to the case."
        ),
            ...
            if contact:
                ...
                for email in emails:
                    send_participant_email_notification(
                        [email], cp.case, self.cleaned_data["title"], self.cleaned_data["email_body"]
                    )
        else:
            vcuser = User.objects.using("vincecomm").filter(username=cp.user_name).first()
            if vcuser:
                send_participant_email_notification(
                    [cp.user_name], cp.case, self.cleaned_data["title"], self.cleaned_data["email_body"]
                )
                followup = add_participant_vinny_case(cp.case, cp)
                followup.comment = self.cleaned_data["email_body"]
                followup.save()
...
class VendorNotificationForm(forms.ModelForm):
    email_body = forms.CharField(
        ...
        label=_("Email Body"),
        help_text=_(
            "This email template (vendor_notify_newcase) already contains a greeting (Hello), the team signature (configurable in team settings), and a link to the case."
        ),
        initial=settings.STANDARD_VENDOR_EMAIL,
        required=True,
    )
    email_template = forms.ChoiceField(
        choices=(), label="Choose an email template to send to the vendor(s)", required=False
    )
    ...
    def save(self, user=None):
        email = VendorNotificationEmail(
            subject=self.cleaned_data["subject"], email_body=self.cleaned_data["email_body"]
        )
        email.save()
...
class ShareVulNoteForm(forms.Form):
    content = forms.CharField(
        label="Post Content", required=False, widget=forms.Textarea(attrs={"class": "form-control"})
    )
...
class UploadFileForm(forms.ModelForm):
    file = forms.FileField(
        required=False, label=_("Upload file"), widget=forms.FileInput(attrs={"class": "vulupload"})
    )
    ...
    def save(self):
        ...
        if file.size:
            ...
            vf = VinceFile(user=self.user, case=case, filename=att.filename, vulnote=vulnote, comm_id=attach.id)
            vf.save()
...
class AddArtifactForm(forms.ModelForm):
    taggles = forms.CharField(max_length=200, label="Tag(s)", required=False)
    ...
    def save(self, ticket=None, case=None, user=None):
        ...
        if ticket:
            followup = create_followup(
                ticket=ticket, title="Added artifact", comment=self.cleaned_data["title"], artifact=artifact, user=user
            )
            followup.save()
...
class CaseFilterForm(forms.Form):
    ...
    tag = forms.CharField(max_length=50, label="Tag", required=False, widget=forms.HiddenInput())
...
class AbstractTicketForm(forms.Form):
    ...
    queue = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "form-control"}), label=_("Queue"), required=True, choices=()
    )
...
class TicketForm(AbstractTicketForm):
    ...
    assigned_to = forms.ChoiceField(
        ...
        label=_("Assign a User"),
        help_text=_(
            "If you select an owner other than yourself, they'll be " "e-mailed details of this ticket immediately."
        ),
    )
    ...
    case = forms.CharField(
        ...
        required=False,
        help_text=_(
            f"If this ticket is associated with a specific Case, add it here. [{settings.CASE_IDENTIFIER}nnnnnn]"
        ),
    )
    ...
    def save(self, user=None):
        ...
        if self.cleaned_data["assigned_to"]:
            title = _("Ticket Opened & Assigned to %(name)s") % {"name": ticket.get_assigned_to or _("<invalid user>")}
        else:
...
class CloseTicketForm(forms.Form):
    ...
    close_choice = forms.ChoiceField(choices=Ticket.CLOSE_CHOICES, label=_("Provide reason"), required=False)
...
WHY_NOT_CHOICES = [
    (1, "Reporter has not attempted to contact any vendors"),
...
class CreateCaseRequestForm(forms.ModelForm):
    ...
    create_case = forms.IntegerField(widget=forms.HiddenInput(), required=False)
    ...
    why_no_attempt = forms.ChoiceField(
        ...
        label="Why has the vendor not been contacted directly?",
        widget=forms.RadioSelect(attrs={"class": "ul_nobullet"}),
        required=False,
    )
    ...
    vul_public = forms.ChoiceField(
        choices=[(True, "Yes"), (False, "No")], label="Is this vulnerability publicly known?", required=False
    )
    ...
    ics_impact = forms.BooleanField(label="Significant ICS/OT impact?", required=False)

    ai_ml_system = forms.BooleanField(label="Related to AI/ML systems?", required=False)
    ...
    def save(self, user=None, submission=None, queue=None):
        ...
        newCase = CaseRequest(
            vrf_id=self.cleaned_data["vrf_id"],
...
class CreateCaseForm(forms.ModelForm):
    ...
    case_request = forms.IntegerField(required=False, widget=forms.HiddenInput())
    ...
    lotus_notes = forms.ChoiceField(
        ...
        help_text=_(
            "If you check No, this case will not be created in VINCEComm (i.e. no collaborative coordination)"
        ),
        required=True,
        widget=forms.RadioSelect(attrs={"class": "ul_nobullet horizontal_bullet"}),
    )
    ...
    def create_action(self, case, title, user=None):
        action = CaseAction(
            case=case, title=title, date=timezone.now(), comment=self.cleaned_data["summary"], action_type=1
        )
        if user:
            action.user = user
        return action
    ...
    def save(self, case=None, user=None, ticket=None):
        ...
        assignment = CaseAssignment(assigned=user, case=newcase)
        assignment.save()
        title = _("Case Opened")
        ...
        if self.cleaned_data["template"]:
            ...
            for task in CaseTask.objects.filter(template=template):
                ...
                fup = FollowUp(ticket=ticket, title=title, date=timezone.now(), comment=task.task_title)
                if user:
                    fup.user = user
                fup.save()
        ...
        if case:
            ...
            fup = FollowUp(ticket=case, title=title, date=timezone.now(), comment=comment)
            if user:
                fup.user = user
            fup.save()
            c = TicketChange(
                followup=fup, field=_("Status"), old_value=old_status_str, new_value=case.get_status_display()
            )
            c.save()
            ...
            if cr:
                ...
                if vc_cr:
                    ...
                    if vc_cr.user:
                        cp = CaseParticipant(case=newcase, user_name=vc_cr.user.username, added_by=user)
                        cp.save()
                        ca = CaseAction(case=newcase, title="Participant Auto-added to Case", user=user, action_type=1)
                        ca.save()
        ...
        if case or ticket:
            ...
            for group in user.groups.exclude(groupsettings__isnull=True):
                ...
                cp = CasePermissions(
                    case=newcase, group=group, group_read=True, group_write=True, publish=group.groupsettings.publish
                )
                cp.save()
...
class EditCaseForm(forms.ModelForm):
    lotus_notes = forms.BooleanField(
        ...
        help_text=_(
            "If you check this box, this case will never be transferred to VINCEComm (i.e. no collaborative coordination)"
        ),
        required=False,
    )
...
class EditCaseRequestForm(forms.ModelForm):
    ...
    why_no_attempt = forms.ChoiceField(
        choices=WHY_NOT_CHOICES,
        label="Why have you not contacted the vendor directly?",
        widget=forms.RadioSelect(attrs={"class": "ul_nobullet"}),
        required=False,
    )
    vul_exploited = forms.ChoiceField(
    ...
    vul_public = forms.ChoiceField(
        choices=[(True, "Yes"), (False, "No")], label="Is this vulnerability publicly known?", required=True
    )
    vul_disclose = forms.ChoiceField(
        choices=[(True, "Yes"), (False, "No")],
        label="Do you plan to publicly disclose this vulnerability yourself?",
...
class GroupForm(forms.ModelForm):
    ...
    def clean_srmail_peer_name(self):
        ...
        if " " in data or "'" in data or "@" in data or "+" in data:
            raise forms.ValidationError(
                "srmail peer names can not contain spaces, quotes, apostrophes, @, or +symbols"
            )
        return data
    ...
    class Meta:
        ...
        widgets = {"comment": forms.Textarea(attrs={"rows": 4}), "version": forms.HiddenInput()}


class InitContactForm(forms.ModelForm):
    contact = forms.CharField(max_length=250, label=_("Contact"), help_text=_("The name of the vendor to verify with"))
    ...
    email_body = forms.CharField(
        widget=forms.Textarea(), label=_("Email Body"), required=True, initial=settings.STANDARD_EMAIL_SIGNATURE
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "countrycode": CountrySelectWidget(
                layout='{widget}</div><div class="large-1 medium-1 columns"><img class="country-select-flag" id="{flag_id}" style="margin: 16px 4px 0" src="{country.flag}"></div>'
            ),
            "version": forms.HiddenInput(),
        }
...
class EditCaseTemplateTaskForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"description": forms.Textarea(attrs={"class": "form-control"}), "template": forms.HiddenInput()}
...
class AddVulnerabilityForm(forms.ModelForm):
    ...
    taggles = forms.CharField(max_length=300, label="Tag(s)", required=False)
...
class AddCVECWEForm(forms.Form):
    cwe = forms.CharField(max_length=500, required=True)
...
class CVEAllocationForm(forms.ModelForm):
    vul = forms.IntegerField(required=False, widget=forms.HiddenInput())
...
class ProductForm(forms.Form):
    version_affected = forms.ChoiceField(
        ...
        choices=(
            [
                ("None", None),
                ("lessThan", "< (affects X versions prior to n)"),
                ("lessThanOrEqual", "<= (affects X versions up to n)"),
            ]
        ),
    )
    ...
    cve_affected_product = forms.CharField(max_length=100, required=False)

    version_name = forms.CharField(max_length=100, required=False)
    version_value = forms.CharField(max_length=100, required=True)
...
class CVEWorkaroundForm(forms.Form):
    workaround = forms.CharField(max_length=1000, label="Describe workaround")
...
class CreateNewVinceUser(forms.Form):
    ...
    preferred_username = forms.RegexField(
        ...
        max_length=254,
        help_text=_(
            "The name visible to other VINCE users. It may only contain 1 space and may not contain certain special characters. (The user can modify this later)"
        ),
        regex=r"^[\w\+-_]+(\s[\w\+-_]+)*$",
        required=True,
        error_messages={
            "invalid": _(
                "Invalid username. The display name may only contain 1 space and may not contain certain special characters."
            )
        },
    )
    ...
    password2 = forms.CharField(max_length=50, required=True, widget=forms.PasswordInput, label="Re-type Password")
...
class NewEmailAll(forms.ModelForm):
    to_group = forms.ChoiceField(
        ...
        choices=(
            [
                (1, "All VINCE Vendor Users"),
                (2, "All Admins"),
                (3, "All Users"),
                (4, "All Staff"),
                (5, "Vendors without Users"),
            ]
        ),
    )
    ...
    subject = forms.CharField(
        max_length=200,
        help_text=_(
            "The subject of the email. If related to a case or ticket, Case ID or Ticket ID will be prepended to subject."
        ),
    )
    ...
    ticket = forms.CharField(max_length=200, label=_("Ticket ID"), required=False)

    case = forms.CharField(
        max_length=200, label=_(f"Is this related to a case? [{settings.CASE_IDENTIFIER}nnnnnn]"), required=False
    )
    ...
    def save(self, user, commit=True):
        ...
        email = VinceEmail(user=user, to=to, ticket=tkt, notification=notification)
        email.save()
...
class NewVinceEmail(forms.ModelForm):
    ...
    email_template = forms.ChoiceField(choices=(), label="Choose an email template", required=False)
    ...
    contact = forms.CharField(
        max_length=250, label=_("Contact"), required=False, help_text=_("Auto-add emails by choosing Contact")
    )
    ...
    pgp_key = forms.CharField(widget=forms.Textarea(), required=False, label=_("Full PGP Key"))
...
class CognitoUserProfile(forms.Form):
    ...
    country = CountryField(default="US").formfield(disabled=True, required=False)

    phone_number = forms.CharField(widget=forms.TextInput(), disabled=True, required=False, max_length=60)

    email = forms.EmailField(required=True, label="Login Username/Email. This field is case sensitive.")
    preferred_username = forms.RegexField(
        label=_("Preferred Display Name"),
        max_length=254,
        help_text=_(
            "The name displayed to other VINCE users. It may only contain 1 space and may not contain certain special characters."
        ),
        regex=r"^[\w\+-_]+(\s[\w\+-_]+)*$",
        required=True,
        error_messages={
            "invalid": _(
                "Invalid username. Your display name may only contain 1 space and may not contain certain special characters."
            )
        },
    )

    org = forms.CharField(max_length=200, label="Company/Affiliation", required=False, disabled=True)
...
class AddRoleUserForm(forms.ModelForm):
    weight = forms.IntegerField(max_value=5, min_value=1)
...
class CVEReserveForm(forms.Form):
    ...
    year = forms.ChoiceField(
        choices=(), widget=forms.Select(attrs={"class": "form-control"}), required=True, label=_("Year")
    )

    count = forms.IntegerField(required=False, label=_("Count"), max_value=10, min_value=1)
...
class CVEFilterForm(forms.Form):
    ...
    year = forms.CharField(
        max_length=5, label="Year", widget=forms.TextInput(attrs={"placeholder": "Year"}), required=False
    )
