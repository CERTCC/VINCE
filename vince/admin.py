...
class BulkAssignmentForm(ActionForm):
    try:
        USER_CHOICES = [(0, "--------")] + [
            (q.id, q.usersettings.preferred_username) for q in get_user_model().objects.all()
        ]
    except:
        USER_CHOICES = []

    user = forms.ChoiceField(choices=USER_CHOICES, label=_("Assign a User"), required=False)

    status = forms.ChoiceField(choices=Ticket.STATUS_CHOICES, label=_("Change Ticket Status"), required=False)

    case = forms.CharField(label=_("Case ID Number"), required=False)
...
class CaseBulkAssignmentForm(ActionForm):
    ...
    status = forms.ChoiceField(choices=VulnerabilityCase.STATUS_CHOICES, label=_("Change Case Status"), required=False)
...
class VinceCaseAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "status",
        "product_name",
        "case_get_assigned_to",
    )
    inlines = [
        CasePermissionInline,
        CaseParticipantInline,
    ]
...
admin.site.login = staff_member_required(
    COGLoginView.as_view(template_name="vince/admin_login.html"), login_url=settings.LOGIN_URL
)
