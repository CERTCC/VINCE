...
from vince.lib import (
    ...
    update_vinny_case,
    get_triage_users,
    add_coordinator_case,
)
from vince.lib import (
    add_vendor_vinny_case,
    remove_vendor_vinny_case,
    remove_participant_vinny_case,
    get_casemember_from_vc,
    get_bounce_stats,
)
from vince.mailer import (
    safe_template_context,
    send_email_notification,
    send_updatecase_mail,
    send_participant_email_notification,
    send_user_approve_notification,
    send_submitter_email_notification,
    send_encrypted_mail,
    send_approval_email,
    send_vendor_approval_emails,
    send_regular_email_notification,
    send_smime_encrypted_mail,
...
from cogauth.utils import (
    ...
    cognito_check_track_permissions,
    disable_totp_mfa,
    disable_sms_mfa,
    get_user_details,
    send_courtesy_email,
    create_new_user,
    admin_change_user_details,
)
        ...
        try:
            ...
            if hasattr(e, "response") and e.response:
                ...
                if hasattr("status_code", r) and r.status_code == 401:
                    logger.warning(
                        f"Authorization failed for {acc.email}, disabling this CVE API account to avoid further errors"
                    )
                    acc.active = False
                    acc.save()
    else:
...
def prepare_info_for_report_element(self):
    info_for_context = {}
    ...
    if month == 0:
        ...
        month = 1
        year = year + 1
    info_for_context["year"] = year
    info_for_context["monthstr"] = date(year, month, 1).strftime("%B")
    info_for_context["month"] = month
    info_for_context["teams"] = Group.objects.exclude(groupsettings__contact__isnull=True)
    user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
    if self.kwargs.get("pk"):
        info_for_context["my_team"] = Group.objects.get(id=self.kwargs.get("pk"))
        info_for_context["teams"] = info_for_context["teams"].exclude(id=self.kwargs.get("pk"))
    else:
        info_for_context["my_team"] = user_groups[0]
        info_for_context["teams"] = info_for_context["teams"].exclude(id=info_for_context["my_team"].id)
        # this team's queues

    my_queues = get_team_queues(info_for_context["my_team"])

    return [info_for_context, my_queues]
        ...
        if "field" in key:
            ...
            if field == "vu":
                ...
                if value in ["Unassigned", "unassigned"]:
                    # all assigne cases
                    assigned = (
                        CaseAssignment.objects.filter(case__in=cases).distinct().values_list("case__id", flat=True)
                    )
                    cases = cases.exclude(id__in=assigned)
                else:
                    assigned = CaseAssignment.objects.filter(
                        assigned__in=team, case__status=1, assigned__usersettings__preferred_username__icontains=value
...
def autocomplete_contact(request, name):
    ...
    if contact:
        ...
        emails = ",".join(emails)
        admin_emails = [admin.email.email for admin in GroupAdmin.objects.filter(contact=contact.id)]
        allemails = contact.emailcontact_set.all()
        for email in allemails:
            if email.email_list:
                admin_emails.append(email.email)
        admin_emails = ",".join(admin_emails)
        pgp_keys = list(
            ContactPgP.objects.filter(contact=contact, revoked=False)
            .exclude(pgp_key_data__isnull=True)
            .exclude(pgp_key_data__exact="")
            .values_list("pgp_key_id", flat=True)
        )
        pgp_key_info = list(
            ContactPgP.objects.filter(contact=contact, revoked=False).values_list(
                "pgp_key_id", "pgp_email", "pgp_key_data", "startdate", "enddate"
            )
        )
        ...
        data = {
            ...
            "pgp": pgp_keys,
            "pgp_key_data": pgp_key_data,
            "pgp_key_info": pgp_key_info,
        }
    data = json.dumps(data)
    mimetype = "application/json"
    return HttpResponse(data, mimetype)
...
def autocomplete_cwe(request):
    if request.GET.get("term"):
        cwe = list(
            CWEDescriptions.objects.filter(cwe__icontains=request.GET.get("term")).values_list("cwe", flat=True)
        )
    else:
...
def calendar_events(request):
    ...
    events = CalendarEvent.objects.filter(Q(date__range=(start, end)) | Q(end_date__range=(start, end))).filter(
        user__in=users
    )
...
def autocomplete_casetasks(request, pk):
    ...
    page = request.GET.get("page", 1)
    tickets = (
        Ticket.objects.filter(case=case)
        .annotate(
            custom_order=DBCase(
                When(status=Ticket.OPEN_STATUS, then=Value(1)),
...
def autocomplete_users(request):
    userlist = list(
        User.objects.using("vincecomm").all().order_by("username").values_list("username", flat=True).distinct()
    )
...
def autocomplete_assignable_users(request):
    assignable_users = list(
        User.objects.filter(is_active=True, groups__name="vince")
        .order_by("usersettings__preferred_username")
        .values_list("usersettings__preferred_username", flat=True)
        .exclude(usersettings__preferred_username__isnull=True)
        .distinct()
    )
...
def autocomplete_tags(request):
    if request.GET.get("term"):
        caselist = list(
            ArtifactTag.objects.filter(tag__icontains=request.GET.get("term"))
            .order_by("tag")
...
def _add_group_permissions(email, vince_user):
    email_contact = (
        VinceCommEmail.objects.using("vincecomm")
        .filter(email__iexact=email)
        .exclude(Q(email_list=True) | Q(status=False))
    )
    ...
    for contact in email_contact:
        ...
        if user:
            ...
            if group:
                ...
                gc = GroupContact(group=group, contact=contact.contact)
    ...
    if user:
        ...
        if group:
            if user.groups.filter(name=group.name).exists():
                ...
                _add_activity(
                    vince_user, 5, vtcontact, f"removing VINCE user {email} from group {vtcontact.vendor_name}"
                )
...
def calc_basic_ticket_stats(Tickets):
    ...
    average_nbr_days_until_ticket_closed_last_14_days = calc_average_nbr_days_until_ticket_resolved(
        all_closed_last_14_days
    )

    # put together basic stats
    basic_ticket_stats = {
        "average_nbr_days_until_ticket_closed": average_nbr_days_until_ticket_closed,
        "average_nbr_days_until_ticket_closed_last_14_days": average_nbr_days_until_ticket_closed_last_14_days,
...
class RemindersView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["other_reminders"] = (
            VinceReminder.objects.filter(created_by=self.request.user)
...
class NewReminderView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        form.fields["user"].choices = [(self.request.user.id, "Myself")] + [
...
class PreferenceView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_initial(self):
        queues = (
            TicketQueue.objects.filter(
                queuepermissions__group__in=self.request.user.groups.all(), queuepermissions__group_read=True
            )
            .order_by("slug")
            .distinct()
        )
...
class TeamDashView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        queues = TicketQueue.objects.filter(
            queuepermissions__group__id=context["team"].id, queuepermissions__group_write=True
        ).distinct()
        tickets = Ticket.objects.filter(queue__in=queues).order_by("modified")
        context["tickets"] = tickets.exclude(
...
class DashboardView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["publishedcases"] = (
            CaseAssignment.objects.filter(assigned=self.request.user, case__status=1)
            .exclude(case__vulnote__date_published__isnull=True)
            .order_by("case__due_date")
        )
        context["cases"] = CaseAssignment.objects.filter(
            assigned=self.request.user, case__status=1, case__vulnote__date_published__isnull=True
        ).order_by("case__due_date")
...
class DashboardStatsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        tickets = (
            ...
            .exclude(status__in=[Ticket.CLOSED_STATUS, Ticket.RESOLVED_STATUS, Ticket.DUPLICATE_STATUS])
            .order_by("modified")
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        cases = CaseAssignment.objects.filter(
            assigned=self.request.user, case__status=1, case__vulnote__date_published__isnull=True
        ).order_by("case__due_date")
                ...
                if posts:
                    ...
                    for post in posts:
                        ...
                        try:
                            postsjs.append(
                                {
                                    "url": reverse("vinny:case", args=[case.id]),
                                    "case": f"{settings.CASE_IDENTIFIER}{post.case.vuid}",
            ...
            return JsonResponse(
                {
                    ...
                    "remove_link": reverse("vince:unattachfile", args=[vc.id]),
                },
                status=200,
            )
        else:
...
class AllResults(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if facet == "All":
            ...
            activity_results = (
                FollowUp.objects.filter(ticket__queue__in=my_queues)
                .filter(Q(title__icontains=search_term) | Q(comment__icontains=search_term))
            ...
            email_contacts = EmailContact.objects.filter(
                contact__vendor_type="Contact", email__in=user_contacts
            ).values_list("contact__id", flat=True)
            email_results = (
                EmailContact.objects.filter(Q(email__icontains=search_term) | Q(name__icontains=search_term))
                .exclude(contact__id__in=email_contacts)
            ...
            cve_results = CVEAllocation.objects.extra(
                where=["search_vector @@ (to_tsquery('english', %s))=true"], params=[search_query]
            )

            vulnote_results = VulNoteRevision.objects.extra(
                where=["search_vector @@ (to_tsquery('english', %s))=true"], params=[search_query]
            ).values_list("vulnote__case__id", flat=True)
            if vulnote_results:
        ...
        qs = sorted(results, key=lambda x: normalize_time(x, "modified"), reverse=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        res = (
            Ticket.objects.filter(
                queue__queuepermissions__group__in=self.request.user.groups.all(),
                queue__queuepermissions__group_read=True,
            )
            .order_by("-modified")
            .distinct()
        )
        ...
        if "contact" in self.request.POST:
            if self.request.POST["contact"]:
                tkts = TicketContact.objects.filter(contact__vendor_name=self.request.POST["contact"]).values_list(
                    "ticket", flat=True
                )
        ...
        if "wordSearch" in self.request.POST:
            if self.request.POST["wordSearch"]:
                ...
                tkttags = list(
                    TicketTag.objects.filter(tag__in=tags, ticket__in=res).values_list("ticket__id", flat=True)
                )
                if tktsearch:
                ...
                titlesearch = list(
                    res.filter(title__icontains=self.request.POST["wordSearch"]).values_list("id", flat=True)
                )
                res = list(
                    res.extra(
                        where=["search_vector @@ (to_tsquery('english', %s))=true"], params=[wordSearch]
                    ).values_list("id", flat=True)
                )

                activity_results = list(
                    FollowUp.objects.filter(
                        Q(title__icontains=self.request.POST["wordSearch"])
                        | Q(comment__icontains=self.request.POST["wordSearch"])
                    )
                    .filter(ticket__id__in=tkt_ids)
                    .values_list("ticket", flat=True)
                )
        ...
        if self.request.POST.get("contact") or self.request.POST.get("submitted_by"):
            ...
            return render(
                request, self.template_name, {"object_list": paginator.page(page), "total": res.count(), "form": form}
            )
...
class ActivityFilterResults(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def get_queryset(self):
        user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
        case_perms = CasePermissions.objects.filter(group_read=True, group__in=user_groups).values_list(
            "case", flat=True
        )
        ca = list(CaseAction.objects.filter(case__in=case_perms).values_list("action_ptr_id", flat=True))
        queue_perms = QueuePermissions.objects.filter(group_read=True, group__in=user_groups).values_list(
            "queue", flat=True
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(
            request, self.template_name, {"activity": paginator.page(page), "paginator": 1, "total": res.count()}
        )
...
class TicketFilter(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        readable_queues = TicketQueue.objects.filter(
            queuepermissions__group__in=self.request.user.groups.all(), queuepermissions__group_read=True
        ).distinct()
        ...
        form.fields["team"].choices = [
            (g.id, g.name) for g in Group.objects.filter(groupsettings__contact__isnull=False)
        ]

        team_memberships = []
        for g in Group.objects.filter(groupsettings__contact__isnull=False):
            team_membership = {
                "group_id": g.id,
                "group_name": g.name,
                "users": [{"id": u.id, "user_name": f"{u.first_name} {u.last_name}"} for u in g.user_set.all()],
            }
            team_memberships.append(team_membership)
        context["team_memberships_js"] = team_memberships
...
class AddCaseArtifactView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        for tag in tags:
            tag = ArtifactTag(artifact=artifact, tag=tag, user=self.request.user)
            tag.save()
...
class CaseTaskFilter(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        res = res.filter(
            Q(description__icontains=self.request.POST["keyword"])
            | Q(submitter_email__icontains=self.request.POST["keyword"])
            | Q(title__icontains=self.request.POST["keyword"])
        )
...
class CommunicationsFilterResults(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if vendorlist:
            ...
            if vcmembers_list:
                vcmembers_list.extend(
                    list(User.objects.using("vincecomm").filter(groups__in=vc_groups).values_list("id", flat=True))
                )
            else:
                vcmembers_list = list(
                    User.objects.using("vincecomm").filter(groups__in=vc_groups).values_list("id", flat=True)
                )
        ...
        if commslist:
            ...
            ca = Action.objects.select_related("caseaction").filter(
                caseaction__case=case, caseaction__action_type__in=commslist
            )
            ta = []
            if "8" in commslist:
        ...
        activity = sorted(case_activity, key=lambda x: normalize_time(x, "created"), reverse=True)

        paginator = Paginator(activity, 10)
        return render(
            request,
            self.template_name,
            {"activity": activity, "total": len(activity), "case": case, "allow_edit": True},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "owner" in self.request.POST:
            ...
            if "0" in ownerlist:
                ...
                all_unassigned_cases = list(
                    VulnerabilityCase.objects.exclude(id__in=all_assigned_cases).values_list("id", flat=True)
                )
        ...
        return render(
            request,
            self.template_name,
            {"object_list": paginator.page(page), "total": res.count(), "form": form, "case": 1},
        )
...
class CaseFilter(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"] = form
        logger.debug(f"in CaseFilter, context is {context}")
        return context
...
class EditCaseRequestView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ticket = get_object_or_404(Ticket, id=self.kwargs["ticket_id"])
        cr = get_object_or_404(CaseRequest, id=ticket.id)
        if vinceutils.deepGet(cr.metadata, "ai_ml_system") == True:
...
class AssignTicketNewTeam(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        form.fields["team"].choices = [
            (g.id, g.name) for g in Group.objects.exclude(groupsettings__contact__isnull=True)
        ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if ticket.case:
                ...
                if CasePermissions.objects.filter(
                    case=ticket.case, group=new_group, group_read=True, group_write=True
                ).exists():
                    ...
                    new_queue = QueuePermissions.objects.filter(
            ...
            if not tqa:
                new_queue = QueuePermissions.objects.filter(
                    queue__queue_type=TicketQueue.GENERAL_TICKET_QUEUE,
                    group__in=[new_group],
                    group_read=True,
                    group_write=True,
                ).first()
            ...
            if tqa:
                    ...
                    if cr.vc_id:
                        ...
                        if vtcr:
                            ...
                            group = (
                                ...
                                .filter(name=new_group.groupsettings.contact.uuid)
                                .first()
                            )
                            if group:
                ...
                messages.success(self.request, _(f"Ticket has been reassigned to {new_group.name}"))
                return redirect("vince:dashboard")
        logger.debug(f"Errors in {self.__class__.__name__} is {form.errors}")
        if error:
            messages.error(self.request, _(f"{error}"))
        else:
            messages.error(self.request, _(f"Error reassigning to new team"))
...
class EditTicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    ContactFormSet = inlineformset_factory(
        Ticket, TicketContact, form=TicketContactForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "vince/edit_ticket.html",
                {"ticket": ticket, "form": form, "ticketpage": 1, "ticket_id": ticket.id},
            )
        # return super().form_invalid(form)
    ...
    def get_context_data(self, **kwargs):
        ...
        user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
        writable_queues = TicketQueue.objects.filter(
            queuepermissions__group__in=user_groups, queuepermissions__group_write=True
        ).distinct()
        if ticket.case:
            queue = get_case_case_queue(ticket.case)
            form.fields["queue"].choices = [(queue.id, queue.title)]
        else:
            form.fields["queue"].choices = [("", "--------")] + [(q.id, q.title) for q in writable_queues]
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["assigned_to"] == "-2":
            ...
            if assignment:
                ...
                form._errors.setdefault("assigned_to", ErrorList(["There are no users available for this role"]))
                return super().form_invalid(form)
            ...
            if not has_queue_write_access(u, queue):
                form._errors.setdefault("assigned_to", ErrorList(["User does not have access to ticket queue."]))
                return self.form_invalid(form)
...
class CreateNewCaseRequestView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        queue = TicketQueue.objects.filter(
            queuepermissions__group__in=user_groups,
            queuepermissions__group_write=True,
            queue_type=TicketQueue.CASE_REQUEST_QUEUE,
        ).first()
        ...
        if self.request.POST:
            ...
            if "ticket_id" in self.kwargs:
                ...
                ticket = Ticket(
                    title=f"Case Request for {case.vu_vuid}",
    ...
    def form_valid(self, form):
        context = form.cleaned_data
        logger.debug(f"in {self.__class__.__name__} context is {context}")
        caserequest = form.save(user=self.request.user)
        if "ai_ml_system" in context:
            logger.debug(f"In {self.__class__.__name__} ai_ml_system is {context['ai_ml_system']}")
            if not caserequest.metadata:
                caserequest.metadata = {"ai_ml_system": context["ai_ml_system"]}
            else:
                caserequest.metadata["ai_ml_system"] = context["ai_ml_system"]
        caserequest.save()
        logger.debug(f"CreateNewRequestView has been triggered with form_valid and caserequest is {caserequest}")
        if caserequest.case:
...
class CreateNewCaseView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
        ...
        if self.request.POST:
            ...
            if "ticket_id" in self.kwargs:
                ...
                if ticket:
                    # if ticket:
                    cr["summary"] = (
                        (ticket.vul_description[:995] + "..")
                        if len(ticket.vul_description) > 995
                        else ticket.vul_description
                    )
                    ...
                    if ticket:
                        cr["summary"] = (
                            (ticket.description[:995] + "..") if len(ticket.description) > 995 else ticket.description
                        )
                        cr["vuid"] = generate_vuid()
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["auto_assign"] == "True":
            ...
            assignment = auto_assignment(role.id)
            if assignment == None:
                form._errors.setdefault("auto_assign", ErrorList(["There are no users available for this role"]))
                return super().form_invalid(form)

        if self.request.POST.get("vuid"):
            old_case = VulnerabilityCase.objects.filter(vuid=self.request.POST["vuid"]).first()
            if old_case:
                form._errors.setdefault("vuid", ErrorList(["Case ID Collision Error"]))
                return super().form_invalid(form)
        saved = False
        if "ticket_id" in self.kwargs:
        ...
        if "ticket_id" in self.kwargs:
            ...
            if ticket.queue.team:
                if self.request.user.groups.filter(id=ticket.queue.team.id).exists():
                    ...
                    ticket.queue = TicketQueue.objects.filter(
                        queue_type=TicketQueue.CASE_TASK_QUEUE, team=ticket.queue.team
                    ).first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        form_class = self.get_form_class()
        form = self.get_form(form_class)
        form.fields["template"].choices = [("", "---------")] + [(q.id, q.title) for q in CaseTemplate.objects.all()]
        user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
        roles = UserRole.objects.filter(Q(group__in=user_groups) | Q(group__isnull=True))
        form.fields["role"].choices = [(q.id, q.role) for q in roles]
        if form.is_valid():
...
class ShareVulNote(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not vulnote.date_shared:
            if vulnote.case.due_date:
                initial = {
                    "content": f"Please [view this draft vulnerability note]({settings.KB_SERVER_NAME}{link_to_vulnote}) that we expect to publish on {vulnote.case.due_date.strftime('%Y-%m-%d')}."
                }
            else:
                initial = {
                    "content": f"Please [view this draft vulnerability note]({settings.KB_SERVER_NAME}{link_to_vulnote})."
                }
            context["form"] = ShareVulNoteForm(initial=initial)
        else:
            initial = {
                "content": f"The [draft vulnerability note]({settings.KB_SERVER_NAME}{link_to_vulnote}) has been updated."
            }
            context["form"] = ShareVulNoteForm(initial=initial)
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=vulnote.case,
            title="Shared Vulnerability Note in VinceComm",
            user=self.request.user,
            vulnote=vulnote.current_revision,
        ...
        if self.request.POST["content"] != "":
            notification = VendorNotificationContent(
    ...
    def post(self, request, *args, **kwargs):
        ...
        for file in staticfiles:
            ...
            try:
                ...
                messages.error(self.request, _(f"Error copying file {vt_file.file.filename}. Publish failed."))
                return HttpResponseRedirect(reverse("vince:case", args=[vulnote.case.id]) + "#vulnote")
        ...
        try:
            ...
            messages.error(self.request, _("Write error occurred while publishing your vulnerability note."))
            return HttpResponseRedirect(reverse("vince:case", args=[vulnote.case.id]) + "#vulnote")
        else:
            messages.success(self.request, _("Congratulations! Your vul note has been published."))

        ca = CaseAction(
            case=vulnote.case,
            title="Published Vulnerability Note",
            user=self.request.user,
            vulnote=vulnote.current_revision,
...
def share_vulnote(vulnote):
    ...
    if vpnote:
        ...
        note, created = VCVulnerabilityNote.objects.update_or_create(
        ...
        vpnote, created = VCVUReport.objects.update_or_create(
            idnumber=note.vuid,
    ...
    for vul in case.vulnerability_set.all():
        if vul.deleted:
            ...
            if vp_vul:
                vp_vul.delete()
            continue
        vp_vul = VCNoteVulnerability.objects.update_or_create(
            case_increment=vul.case_increment,
            note=vpnote.vulnote,
            defaults={"cve": vul.cve, "description": vul.description, "uid": vul.uid},
        )
    for vtvendor in case.vulnerablevendor_set.all():
        if vtvendor.deleted:
            vp_vendor = VCVendor.objects.filter(note=vpnote.vulnote, uuid=vtvendor.contact.uuid).first()
            ...
            vp_vendor, created = VCVendor.objects.update_or_create(
                ...
                defaults={
                    "contact_date": vtvendor.contact_date,
            ...
            vp_vendor, created = VCVendor.objects.update_or_create(
                note=vpnote.vulnote,
                uuid=vtvendor.contact.uuid,
                defaults={"contact_date": vtvendor.contact_date, "vendor": vtvendor.contact.vendor_name},
            )
        ...
        for status in vtvendor.vendorstatus_set.all():
            if not status.vul.deleted and status.approved:
                vp_vul = VCNoteVulnerability.objects.filter(
                    case_increment=status.vul.case_increment, note=vpnote.vulnote
                ).first()
                if vp_vul:
                    if vtvendor.share:
                        ...
                        vp_vulstatus = VCVendorVulStatus.objects.update_or_create(vendor=vp_vendor, vul=vp_vul)
...
def copy_vulnote(vulnote, database):
    ...
    if vpnote:
        ...
        for vul in case.vulnerability_set.all():
            if vul.deleted:
                vp_vul = (
                    NoteVulnerability.objects.using(database)
                    .filter(case_increment=vul.case_increment, note=vpnote.vulnote)
                    .first()
                )
                if vp_vul:
            ...
            vp_vul = NoteVulnerability.objects.using(database).update_or_create(
                case_increment=vul.case_increment,
                note=vpnote.vulnote,
                defaults={"cve": vul.cve, "description": vul.description, "uid": vul.uid},
            )
        for vtvendor in case.vulnerablevendor_set.all():
            ...
            if vtvendor.deleted:
                vp_vendor = (
                    Vendor.objects.using(database).filter(note=vpnote.vulnote, uuid=vtvendor.contact.uuid).first()
                )
                if vp_vendor:
                ...
                if vp_vendor:
                    ...
                    if (
                        (vtvendor.references != vp_vendor.references)
                        or (vtvendor.statement != vp_vendor.statement)
                        or (vtvendor.statement_date != vp_vendor.statement_date)
                        or (vtvendor.addendum != vp_vendor.addendum)
                    ):
                        updated = True
            ...
            for status in vtvendor.vendorstatus_set.all():
                if not status.vul.deleted and status.approved:
                    vp_vul = (
                        NoteVulnerability.objects.using(database)
                        .filter(case_increment=status.vul.case_increment, note=vpnote.vulnote)
                        .first()
                    )
                    if vp_vul:
                        if vtvendor.approved:
                            vp_vulstatus = (
                                VendorVulStatus.objects.using(database).filter(vendor=vp_vendor, vul=vp_vul).first()
                            )
                            if vp_vulstatus:
                                # did status or statement change?
                                if (
                                    (vp_vulstatus.status != status.status)
                                    or (vp_vulstatus.references != status.references)
                                    or (vp_vulstatus.statement != status.statement)
                                ):
                                    updated = True
        ...
        for vul in Vulnerability.casevuls(case):
            vp_vul = NoteVulnerability(
                cve=vul.cve, case_increment=vul.case_increment, description=vul.description, uid=vul.uid, note=vpnote
            )
            vp_vul.save(using=database)
        # add vendors
        for vul in VulnerableVendor.casevendors(case):
            vendor = Vendor(
                note=vpnote, uuid=vul.contact.uuid, vendor=vul.contact.vendor_name, contact_date=vul.contact_date
            )
            if vul.approved:
            ...
            for status in vul.vendorstatus_set.all():
                vp_vul = NoteVulnerability.objects.filter(
                    case_increment=status.vul.case_increment, note=vpnote
                ).first()
                if vp_vul:
                    vp_vulstatus = VendorVulStatus(vendor=vendor, vul=vp_vul, status=status.status)
                    if status.approved:
        ...
        vpnote.save(using=database)
        # create a VUReport Record
        report, created = VUReport.objects.update_or_create(
            idnumber=vpnote.vuid,
...
class VulNoteReviewal(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        check = VulNoteReview.objects.filter(
            vulnote=vulnote.current_revision.id, reviewer=self.request.user, complete=False
        ).first()
        if check:
            initial = {
                "content": check.review,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if vulnote.ticket_to_approve:
            if (not self.request.user.is_superuser) and (
                vulnote.ticket_to_approve.submitter_email == self.request.user
            ):
                messages.error(self.request, _("You are not permitted to approve this vul note."))
                return HttpResponseRedirect(reverse("vince:case", args=[vulnote.case.id]) + "#vulnote")
            ...
            request.POST = {"new_status": Ticket.RESOLVED_STATUS}
...
class AskApprovalVulNote(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ticket.save()
        action = FollowUp(ticket=ticket, title="Request for approval", date=timezone.now(), user=self.request.user)
...
class DownloadVulNote(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        for vul in (
            VulnerableVendor.casevendors(case)
            .filter(vendorstatus__status=VendorStatus.AFFECTED_STATUS)
            .distinct("vendor")
            .order_by("vendor")
        ):
            v = add_vendor(vul)
        ...
        for vul in (
            VulnerableVendor.casevendors(case)
            .filter(vendorstatus__status=VendorStatus.UNAFFECTED_STATUS)
            .distinct("vendor")
            .order_by("vendor")
        ):
            v = add_vendor(vul)
        ...
        for vul in (
            VulnerableVendor.casevendors(case)
            .filter(Q(vendorstatus__status=VendorStatus.UNKNOWN_STATUS) | Q(vendorstatus__isnull=True))
            .distinct("vendor")
            .order_by("vendor")
        ):
            v = add_vendor(vul)
...
class CreateVulNote(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        vulnote = VulNote(case=case, owner=self.request.user)
        vulnote.save()
        vulnote.add_revision(
            VulNoteRevision(
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update(
            {"case": get_object_or_404(VulnerabilityCase, id=self.kwargs["case_id"]), "references": references}
        )
        return kwargs
...
class CaseRequestView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "autoassign" in request.GET:
            ...
            context["form"] = form
            context["ticket"] = ticket
            logger.debug(f"in CaseRequestView the context is {context}")
        ...
        if "take" in request.GET:
            ...
            request.POST = {"owner": request.user.id, "title": ticket.title, "comment": ""}
        ...
        if "assign" in request.GET:
            ...
            request.POST = {"owner": request.GET["assign"], "title": ticket.title, "comment": ""}
            kwargs["ticket_id"] = self.kwargs["pk"]
            return update_ticket(request, self.kwargs["pk"])
    ...
    def get_context_data(self, **kwargs):
        ...
        if vinceutils.deepGet(context["ticket"].metadata, "ai_ml_system") == True:
            context["ai_ml_system"] = True
        else:
            context["ai_ml_system"] = False
        user_groups = (
            context["ticket"]
            .queue.queuepermissions_set.filter(group_read=True, group_write=True)
        ...
        context["other_tags"] = [
            tag.tag
            for tag in TagManager.objects.filter(tag_type=1)
            .filter(Q(team__in=user_groups) | Q(team__isnull=True))
            .exclude(tag__in=context["old_tags"])
            .order_by("tag")
            .distinct("tag")
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["ticket"].followup_set.count() > Ticket.MAX_ACTIVITY:
            if self.request.GET.get("all", None):
                ...
                context["more"] = True
        if CaseRequest.objects.filter(pk=self.kwargs["pk"]):
            cr = CaseRequest.objects.get(pk=self.kwargs["pk"])
            if VTCaseRequest.objects.filter(vrf_id=cr.vrf_id).first():
...
class TicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_subscribers(ticket_id):
        ...
        allusers = [
            ...
            for user in User.objects.filter(is_active=True, groups__name="vince").exclude(
                usersettings__preferred_username__isnull=True
            )
        ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("role"):
            ...
            # update the ticket
            request.POST = {"owner": assignment.id, "title": ticket.title, "comment": "", "auto": 1}
            kwargs["ticket_id"] = self.kwargs["pk"]
...
class UpdateCaseView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def add_tag(case_id, user, tag):
        ...
        if len(tag) < 50:
            if (
                TagManager.objects.filter(tag=tag, tag_type=3)
                .filter(Q(team__in=user_groups) | Q(team__isnull=True))
                .exists()
            ):
                tag, created = CaseTag.objects.update_or_create(case=case, tag=tag, defaults={"user": user})
                if created:
                    fup = CaseAction(title=f'Case tagged as "{tag}"', case=case, user=user)
                    fup.save()
            else:
            ...
            return JsonResponse(
                {"tag": tag, "case": case.id, "error": "Tag is too long. Max 50 characters."}, status=401
            )
    ...
    def del_tag(case_id, user, tag):
        ...
        try:
            ...
            fup = CaseAction(title=f'Removed case tag "{tag}"', case=case, user=user)
            fup.save()
    ...
    def addComment(case_id, user, comment):
        ...
        ca = CaseAction(
            ...
            user=user,
            comment=comment,
            title=f"{user.usersettings.vince_username} added a comment",
            action_type=CaseAction.lookup("VinceTrack"),
        )
        ca.save()
...
class UpdateTicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def add_subscriber(username, ticket_id):
        ...
        if TicketCC.objects.filter(ticket=ticket, user=user):
            return JsonResponse(
                {
                    "subscriber_added": user.username,
                    "ticket": ticket.id,
            ...
            return JsonResponse(
                {
                    "user": user.username,
                    "ticket": ticket.id,
                    "error": f"User '{user.username}' is not subscribed to ticket '{ticket.id}'",
                },
                status=401,
            )
    ...
    def add_tag(tag, ticket_id, user):
        ...
        if len(tag) < 50:
            if (
                TagManager.objects.filter(tag=tag, tag_type=1)
                .filter(Q(team__in=user_groups) | Q(team__isnull=True))
                .exists()
            ):
                tag, created = TicketTag.objects.update_or_create(ticket=ticket, tag=tag, defaults={"user": user})
                if created:
                    fup = FollowUp(title=f'Ticket tagged as "{tag}"', ticket=ticket, user=user)
                    fup.save()
            else:
                logger.debug(f"invalid tag - tag {tag} doesn't exist in tag manager")
            ...
            return JsonResponse(
                {"tag": tag, "ticket": ticket.id, "error": "Tag is too long. Max 50 characters."}, status=401
            )
    ...
    def del_tag(tag, ticket_id, user):
        ...
        try:
            ...
            fup = FollowUp(title=f'Removed ticket tag "{tag}"', ticket=ticket, user=user)
            fup.save()
            return JsonResponse({"tag_deleted": tag, "ticket": ticket.id}, status=200)
        except TicketTag.DoesNotExist:
            return JsonResponse(
                {"tag": tag, "ticket": ticket.id, "error": f"'{tag}' not assigned to ticket '{ticket.id}'"}, status=401
            )
...
class DeleteTicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (self.request.user.is_superuser):
            messages.error(self.request, _("User must be an administrator to perform this action"))
        ...
        if case.owner != form.cleaned_data["owner"]:
            ...
            if case.owner:
                if new_owner:
                    title = (
                        title
                        + f", changed Case Owner from {case.owner.usersettings.vince_username} to {new_owner.usersettings.vince_username}"
                    )
                else:
            ...
            if ca == None:
                ...
                if new_owner:
                    ca = CaseAssignment(case=case, assigned=new_owner)
                    ca.save()
...
class DashboardTicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        if self.kwargs["type"] == "Open":
            ...
            tickets = Ticket.objects.filter(assigned_to=self.request.user, status__in=[1, 2, 6]).values_list(
                "id", flat=True
            )
...
class CaseTicketView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        tickets = (
            ...
            .order_by("custom_order", "-modified")
        )
        ticketsjs = [obj.as_dict() for obj in tickets]
        return JsonResponse(ticketsjs, safe=False, status=200)
...
class DashboardQueueActivityView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["activity"] = FollowUp.objects.filter(
            ticket__status__in=status, ticket__assigned_to=self.request.user
        ).order_by("-date")
        return context
...
class VendorViewDetailView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if vendor.seen:
            context["timeseen"] = CaseAction.objects.filter(
                case=vendor.case, vendor=vendor, action_type=CaseAction.lookup("Vendor Viewed")
            ).first()
        ...
        if vc_contact:
            groupcontact = GroupContact.objects.using("vincecomm").filter(contact=vc_contact).first()
            if groupcontact:
                context["vince_users"] = list(
                    User.objects.using("vincecomm").filter(groups=groupcontact.group).values_list("id", flat=True)
                )
...
class CompleteCaseTransferView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cr = CaseParticipant.objects.filter(
            case=context["case"], status__in=["Lead Requested", "Lead Suggested"]
        ).first()
        if cr:
            # get ticket
    ...
    def post(self, request, *args, **kwargs):
        ...
        # get ticket
        transfer_tkt = Ticket.objects.filter(
            case=case,
            title__icontains="Case Transfer Request",
        ...
        if cp and new_group:
            ...
            ca = CaseAction(
                case=case,
                title=f"Case ownership transferred to {new_group.name}",
                user=self.request.user,
                action_type=CaseAction.lookup("VinceTrack"),
            )
            ca.save()
            ...
            if self.request.user.groups.filter(id=new_group.id).exists():
                newassignment = CaseAssignment.objects.update_or_create(case=case, assigned=self.request.user)
            else:
                # assign requester
                newassignment = CaseAssignment.objects.update_or_create(case=case, assigned=cp.added_by)
            ...
            for c in ca:
                if not (c.assigned.groups.filter(id=new_group.id).exists()):
                    ...
                    act = CaseAction(
                        case=case,
                        title=f"Removing {c.assigned.usersettings.preferred_username} from case assignment due to team transfer",
                        action_type=CaseAction.lookup("VinceTrack"),
                        user=self.request.user,
                    )
                    c.delete()
            ...
            if transfer_tkt:
                fup = FollowUp(title="Transfer approved and completed.", user=self.request.user, ticket=transfer_tkt)
                fup.save()
...
class RejectCaseTransferView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        cp = CaseParticipant.objects.filter(
            case=case, coordinator=True, status__in=["Lead Requested", "Lead Suggested"]
        ).first()
        if cp == None:
            return Http404
        ...
        ca = CaseAction(
            case=case,
            title=f"Case ownership transfer to {new_group.name} rejected.",
...
def _transfer_case(case, new_group):
    ...
    for x in qp:
        cp = CasePermissions.objects.update_or_create(
            group=x.group,
            case=case,
            defaults={"group_read": x.group_read, "group_write": x.group_write, "publish": x.publish},
        )

    # change old team owner to not lead
    cp = CaseParticipant.objects.filter(case=case, coordinator=True, status="Lead")
    for c in cp:
...
class RequestCaseTransferView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["case"].team_owner:
            form.fields["team"].choices = [
                (g.id, g.name)
                for g in Group.objects.exclude(groupsettings__contact__isnull=True).exclude(
                    id=context["case"].team_owner.id
                )
            ]
        else:
            form.fields["team"].choices = [
    ...
    def post(self, request, *args, **kwargs):
        ...
        if case.team_owner:
            form.fields["team"].choices = [
                (g.id, g.name)
                for g in Group.objects.exclude(groupsettings__contact__isnull=True).exclude(id=case.team_owner.id)
            ]
        else:
            form.fields["team"].choices = [
                (g.id, g.name) for g in Group.objects.exclude(groupsettings__contact__isnull=True)
        ...
        if form.is_valid():
            ...
            if CasePermissions.objects.filter(group_write=True, group=new_group, case=case).exists():
                ...
                if has_case_write_access(self.request.user, case):
                    ...
                    cp = CaseParticipant.objects.filter(
                        case=case, user_name=new_group.groupsettings.contact.vendor_name, status="Lead"
                    ).first()
                    if cp:
                        # this group is basically already the lead, make it offical
                        case.team_owner = new_group
                        case.save()
                        messages.success(self.request, "Your request has been approved.")
                        return redirect("vince:case", case.id)

                    ca = CaseAssignment.objects.filter(case=case).first()
                    ticket = Ticket(
                        title=f"Case Transfer Request for {case.vu_vuid}",
                        submitter_email=self.request.user.email,
                        queue=get_case_case_queue(case),
                        case=case,
                    ...
                    # Add case request
                    cp = CaseParticipant.objects.update_or_create(
                        case=case,
                ...
                if self.request.user.groups.filter(id=new_group.id).exists():
                    ...
                    if new_queue:
                        ...
                        cp = CasePermissions.objects.update_or_create(
                            case=case,
                            group=new_group,
                            defaults={"group_read": True, "group_write": False, "publish": False},
                        )

                        messages.success(self.request, "Your request has been sent.")
                        return redirect("vince:case", case.id)

        messages.error(self.request, f"Your request could not be processed. {form.errors}")
...
class CaseActivityView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    model = VulnerabilityCase
    login_url = "vince:login"
    template_name = "vince/include/case_activity.html"

    # Note: this CaseActivityView generates the little form at the top of the Case Activity section,
    # NOT the activity section itself, which is generated by CaseTimelineView.
    ...
    def get_context_data(self, **kwargs):
        ...
        form.fields["vendor"].choices = [(u.id, u.contact.vendor_name) for u in context["vendors"]]
...
class CaseView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for u in users:
            assigned_ordered_pairs.append(
                {"label": u.assigned.usersettings.preferred_username, "value": u.assigned.email}
            )
        ...
        context["case_available_tags"] = [
            tag.tag
            for tag in TagManager.objects.filter(tag_type=3)
            .filter(Q(team__in=user_groups) | Q(team__isnull=True))
            .exclude(tag__in=context["case_tags"])
            .order_by("tag")
            .distinct("tag")
        ]
        context["casepage"] = 1
        ...
        # support is  active
        context["reminders"] = VinceReminder.objects.filter(
            case=context["case"], alert_date__lte=datetime.now(pytz.utc)
        ).order_by("-alert_date")
        context["allow_edit"] = True
        # need this for task reassignment
        context["assignable_users"] = User.objects.filter(is_active=True, groups__name="vince")
        context["assignable_usersjs"] = [{0: "Unassigned"}] + [
            {obj.id: obj.usersettings.preferred_username} for obj in context["assignable_users"]
        ]

        tartifacts = Artifact.objects.select_related("ticketartifact").filter(
            ticketartifact__ticket__in=context["ticket_list"]
        )
        ...
        try:
            ...
            context["revisions"] = VulNoteRevision.objects.filter(vulnote=context["case"].vulnote).order_by(
                "-created"
            )[:4]
        ...
        if cr:
            # get ticket
            context["transfer"] = Ticket.objects.filter(
                case=context["case"],
                title__icontains="Case Transfer Request",
                submitter_email=cr.added_by.email,
                status=Ticket.OPEN_STATUS,
...
class ApproveAllStatements(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        action.save()
        messages.success(self.request, _("All statements have been successfully approved."))
...
class RedirectVinny(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if is_safe_url(next_url, set(settings.ALLOWED_HOSTS), True):
            ...
            logger.info(
                f"The next_url {next_url} is not valid, replacing the next_url field with default VinceComm Dashboard page"
            )
        return context
...
class PushNotification(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(self.request, _("Your post was successfully published."))
...
class DeletePostView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if notification.published:
            messages.error(
                self.request, _("You can not remove a published post from this view.  Remove in VINCEComm.")
            )
        ...
        case = notification.case
        notification.delete()
        messages.success(self.request, _("Your post was successfully removed."))
        ...
        if "notify_id" in self.kwargs:
            ...
            if notification.version != int(self.request.POST["version"]):
                form._errors.setdefault(
                    "version",
                    ErrorList(
                        ["Someone beat you to editing this notification. View the most recent version and retry."]
                    ),
                )
                return super().form_invalid(form)
            ...
            notification = VendorNotificationContent(
                content=form.cleaned_data["content"], case=case, user=self.request.user, post=notifs
            )
            notification.save()
...
class TagUser(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        assignable_users = [
            u.usersettings.preferred_username for u in User.objects.filter(is_active=True, groups__name="vince")
        ]
        return JsonResponse(
            {"response": "success", "case_assigned_to": assignments, "assignable_users": assignable_users}, status=200
        )
        ...
        if self.kwargs.get("case_id"):
            ...
            if int(request.POST["state"]) == 1:
                ...
                if user:
                    ...
                    assignment = CaseAssignment(assigned=user, case=case)
                    assignment.save()
                else:
                ...
                contacts = user.groups.exclude(groupsettings__contact__isnull=True).values_list(
                    "groupsettings__contact__id", flat=True
                )
                cps = CaseParticipant.objects.filter(case=case).filter(
                    Q(user_name=user.email) | Q(contact__in=contacts)
                )
        ...
        if hasattr(context["case"], "case_request") and context["case"].case_request:
            if hasattr(context["case"].case_request, "caserequest"):
                ...
                if hasattr(context["cr"], "vrf_id") and context["cr"].vrf_id:
                    ...
                    if vc_cr:
                        context["vincecomm_link"] = reverse("vinny:cr_report", args=[vc_cr.id])
                if vinceutils.deepGet(context["cr"].metadata, "ai_ml_system") == True:
                    context["ai_ml_system"] = True
        else:
            context["ticket"] = context["case"].case_request
            if hasattr(context["ticket"], "vrf_id") and context["ticket"].vrf_id:
    ...
    def post(self, request, *args, **kwargs):
        ...
        for user in users:
            if user.startswith("Group:"):
                ...
                ca = CaseAction(
                    case=case,
                    title=f"Added Group {groupname} to Case",
                    user=self.request.user,
                    action_type=CaseAction.lookup("VinceTrack"),
                )
                ca.save()
                ...
                for subgroup in groupmembers:
                    ...
                    ca = CaseAction(
                        case=case,
                        title=f"Added Group {subgroup.name} to Case",
                        user=self.request.user,
                        action_type=CaseAction.lookup("VinceTrack"),
                    )
                    ca.save()
                ...
                if vc_user:
                    cp = CaseParticipant.objects.update_or_create(
                        case=case, user_name=user, defaults={"added_by": self.request.user}
                    )
                    ca = CaseAction(
                        case=case,
                        title=f"Added Participant {vc_user.username} to Case",
                        user=self.request.user,
                        action_type=CaseAction.lookup("VinceTrack"),
                    )
                    ca.save()
                    ...
                    if contact:
                        cp = CaseParticipant.objects.update_or_create(
                            ...
                            user_name=user,
                            contact=contact,
                            defaults={"group": True, "added_by": self.request.user},
                        )
                        ca = CaseAction(
                            case=case,
                            title=f"Added Participant Group {user} to Case",
                            user=self.request.user,
                            action_type=CaseAction.lookup("VinceTrack"),
                        )
                        ca.save()
                        ...
                        ca = CaseAction(
                            case=case,
                            title=f"Invited New Participant {user} to Case",
                            user=self.request.user,
                            action_type=CaseAction.lookup("VinceTrack"),
                        )
                        ca.save()
...
def add_vendor_to_case(case, contact, user, group=None):
    ...
    old_vendor = VulnerableVendor.objects.filter(case=case, contact=contact).first()
    if old_vendor == None:
        if contact.active:
            vv = VulnerableVendor(case=case, contact=contact, added_by=user, vendor=contact.vendor_name)
            if group:
                vv.from_group = group
            vv.save()
            ...
            if contact.contacttag_set.count() > 0:
                ...
                if alert:
                    ...
                    queue = get_case_case_queue(case)
                    ticket = Ticket(
                        title=f"Tagged Vendor {contact.vendor_name} added to case",
                        created=timezone.now(),
                        status=Ticket.OPEN_STATUS,
                        queue=queue,
                        assigned_to=user,
...
class CaseVendors(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        summary = VulnerableVendor.casevendors(case).aggregate(
            ...
            needs_approval=Count("id", filter=Q(statement_date__isnull=False, approved=False)),
        )
        for x in summary:
            context[x] = summary[x]
        vcase = Case.objects.using("vincecomm").filter(vuid=case.vuid).first()
        if vcase:
            context["zero_user_vendors"] = (
                CaseMember.objects.using("vincecomm").filter(case=vcase, group__user__isnull=True).count()
            )
        else:
...
class AddVendorToCase(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=case,
            title=f"Added {len(contacts_added)} Vendors to Case",
        ...
        if error:
            return JsonResponse(
                {"status": "false", "message": f"Could not find matching vendor name {error}."}, status=400
            )
        ...
        if contacts_rejected:
            return JsonResponse(
                {
                    "status": "false",
                    "message": f'Will not add the following vendors due to inactive status or previously added to the case: {(", ").join(contacts_rejected)}',
                },
                status=400,
            )
        return JsonResponse({"response": "success"}, status=200)
...
class VendorVulStatement(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _("Got it! Your statement has been recorded."))
        return JsonResponse({"response": "success"}, status=200)
    ...
    if affected:
        for vul in affected:
            ...
            status = VendorStatus.objects.update_or_create(
                vendor=vendor,
                vul=vul_obj,
                defaults={"status": 1, "user_approved": request.user, "approved": True, "user": request.user.username},
            )
            ...
            if member:
                status = CaseMemberStatus.objects.update_or_create(
                    member=member, vulnerability=cv, defaults={"status": 1, "user": vcuser}
                )
    if unknown:
        for vul in unknown:
            vul_obj = Vulnerability.objects.filter(id=int(vul)).first()
            status = VendorStatus.objects.update_or_create(
                vendor=vendor,
                vul=vul_obj,
                defaults={"status": 3, "user_approved": request.user, "approved": True, "user": request.user.username},
            )
            member = get_casemember_from_vc(vendor, vul_obj.case)
            cv = CaseVulnerability.objects.filter(vince_id=int(vul)).first()
            if member:
                status = CaseMemberStatus.objects.update_or_create(
                    member=member, vulnerability=cv, defaults={"status": 3, "user": vcuser}
                )
    if unaffected:
        for vul in unaffected:
            ...
            status = VendorStatus.objects.update_or_create(
                vendor=vendor,
                vul=vul_obj,
                defaults={"status": 2, "user_approved": request.user, "approved": True, "user": request.user.username},
            )
            cv = CaseVulnerability.objects.filter(vince_id=int(vul)).first()
            member = get_casemember_from_vc(vendor, vul_obj.case)
            if member:
                status = CaseMemberStatus.objects.update_or_create(
                    member=member, vulnerability=cv, defaults={"status": 2, "user": vcuser}
                )
...
class EditVendorStatusView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if not (vendor.case.lotus_notes):
            ...
            if member:
                stmt, created = CaseStatement.objects.update_or_create(
                    member=member,
                    case=member.case,
                    defaults={
                        "references": form.cleaned_data["references"],
                ...
                if created:
                    if stmt.statement:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator created case statement"
                        )
                        va.save()
                    ...
                    if stmt.references:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator added case references"
                        )
                        va.save()
                    ...
                    if stmt.addendum:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator added addendum"
                        )
                        va.save()
                    ...
                    if old_statement != stmt.statement:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator updated case statement"
                        )
                        va.save()
                        vs = VendorStatusChange(
                            action=va, field="statement", old_value=old_statement, new_value=stmt.statement
                        )
                        vs.save()
                    if old_references != stmt.references:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator updated references"
                        )
                        va.save()
                        vs = VendorStatusChange(
                            action=va, field="references", old_value=old_references, new_value=stmt.references
                        )
                        vs.save()
                    if old_addendum != stmt.addendum:
                        va = VendorAction(
                            member=member, user=vcuser, case=member.case, title=f"Coordinator updated addendum"
                        )
                        va.save()
                        vs = VendorStatusChange(
                            action=va, field="addendum", old_value=old_addendum, new_value=stmt.addendum
                        )
                        vs.save()
        ...
        messages.success(self.request, _("Got it! The status and statement have been updated."))
...
class ConfirmRemoveParticipant(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=cp.case,
            title="Removed Participant from Case",
        ...
        cp.delete()
        messages.success(self.request, _("Participant successfully removed from case."))
...
class NotifyParticipant(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                messages.error(request, _(f"Invalid Email address: SMTP Recipients Refused {case.user_name}"))
            return HttpResponseRedirect(reverse("vince:case", args=[case.case.id]) + "#participants")
        else:
            messages.error(request, _(f"Invalid Request {form.errors}"))
        return HttpResponseRedirect(reverse("vince:case", args=[case.case.id]) + "#participants")
    ...
    def get_context_data(self, **kwargs):
        ...
        if default_tmpl:
            ...
            context["form"].fields["email_template"].choices = [(default_tmpl.id, default_tmpl.template_name)] + [
                (q.id, q.template_name) for q in templates
            ]
        else:
            context["form"].fields["email_template"].choices = [("", "--------")] + [
                (q.id, q.template_name) for q in templates
            ]
...
class ConfirmRemoveAllVendorsFromCase(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=case,
            title=f"All vendors removed from case",
            user=self.request.user,
            action_type=CaseAction.lookup("VinceTrack"),
        )
        ca.save()
        messages.success(self.request, _("All vendors successfully removed from case."))
...
class ConfirmRemoveVendorFromCase(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=case,
            title=f"Removed Vendor {vendor_name} from Case",
            user=self.request.user,
            action_type=CaseAction.lookup("VinceTrack"),
        )
        ca.save()
        ...
        if vendor.case.published or vendor.case.publicdate:
            ...
            for product in products:
                ...
                for version in versions:
                    affectedproduct = CVEAffectedProduct.objects.filter(
                        name=product.name,
                        version_name=version.version_name,
                        version_affected=version.version_affected,
                        version_value=version.version_value,
                        organization=product.organization.vendor_name,
                    ).first()
            ...
            vendor.delete()
        remove_vendor_vinny_case(case, vendor.contact, self.request.user)
        messages.success(self.request, _("Vendor successfully removed from case."))
...
class NotifyVendorFormView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            {"vendors": vendors, "vn_form": form, "case": case, "vendors_seen": vendors_seen},
        )
...
class EditVendorCaseList(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            ca = CaseAction(
                case=case,
                title=f"Notified Vendors of Case",
                user=self.request.user,
                action_type=CaseAction.lookup("VinceTrack"),
                comment=comment,
            )
        ...
        if attachment:
            if attachment.public:
                ...
                if vinny_case:
                    ...
                    vc_attach = VinceCommCaseAttachment.objects.filter(
                        vince_id=attachment.id, action__case=vinny_case
                    ).first()
                    if vc_attach:
                        vc_attach.delete()
...
class RemoveTicketArtifact(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ticket = get_object_or_404(Ticket, id=self.kwargs["pk"])
        fup = FollowUp(title=f"deleted artifact {artifact.title}", ticket=ticket, user=self.request.user)
        fup.save()
        artifact.delete()
...
def make_artifact_public(request, artifact, case):
    ...
    if case:
        ...
        if ticket:
            if ticket.case:
                ...
                case = ticket.case
    if case_id == 0:
        messages.error(request, _("Can't find related case for this artifact"))
        if ticket:
            return redirect("vince:ticket", ticket.id)
        else:
            return redirect("vince:dashboard")
    ...
    if attachment.public:
        # remove it
        vcattach = VinceCommCaseAttachment.objects.filter(action__case=vinny_case, vince_id=attachment.id)
        ...
        if len(vcattach) == 0:
            attachment.public = False
            attachment.save()
            messages.error(request, _("That artifact isn't available in vincecomm"))
            return redirect("vince:case", case_id)
        messages.success(request, _("Artifact successfully removed"))
    else:
        # addit
        if attachment:
            copy_source = {
            ...
            try:
                ...
                if error_code == "InvalidRequest":
                    ...
                    return JsonResponse(
                        {
                            "status": "success",
                            "text": f"There was an error uploading your file: {e.response['Error']['Code']} {e.response['Error']['Message']}",
                        }
                    )

            action = VendorAction(
                title="Added File",
                user=User.objects.using("vincecomm").filter(username=request.user.email).first(),
                case=vinny_case,
            )
            action.save()
            ...
            # delete the old one
            s3.Object(
                settings.VINCE_SHARED_BUCKET, settings.AWS_PRIVATE_MEDIA_LOCATION + "/" + str(attachment.uuid)
            ).delete()

            add_case = VinceCommCaseAttachment(action=action, file=att, vince_id=attachment.id)
            if add_case:
                add_case.save()
        ...
        messages.success(request, _("Artifact successfully shared"))
    return redirect("vince:case", case_id)
...
class CaseArtifacts(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.debug(f"CaseArtifacts is returning this context: {context}")
        return context
    ...
    def form_valid(self, form):
        ...
        for tag in newtags:
            if tag in oldtags:
                ...
                ntag = ArtifactTag(artifact=artifact, tag=tag, user=self.request.user)
                newt = True
                ntag.save()
        # remove
        for tag in oldtags:
            if tag not in newtags:
                otag = ArtifactTag.objects.filter(artifact=artifact, tag=tag)
                rmt = True
                otag.delete()
        if newt:
        ...
        if ticket:
            ...
            if referer:
                if "case" in referer and ticket.ticket.case:
                    ...
                    tartifacts = Artifact.objects.select_related("ticketartifact").filter(
                        ticketartifact__ticket=ticket.ticket
                    )
            else:
                tartifacts = Artifact.objects.select_related("ticketartifact").filter(
                    ticketartifact__ticket=ticket.ticket
...
class DeleteCaseDependency(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ca = CaseAction(
            case=case, title="Removed Dependency", user=self.request.user, action_type=CaseAction.lookup("VinceTrack")
        )
        ca.save()
...
class RestartContactAssociation(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ticket = req.ticket
        fup = FollowUp(
            title="Contact association failed. Restarting contact association process.",
            user=self.request.user,
            ticket=ticket,
        )
        fup.save()
        ...
        messages.success(self.request, "Success.  Please retry to verify contact.")
        return redirect("vince:ticket", ticket.id)
...
class CompleteContactAssociation(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("remove"):
            ...
            fup = FollowUp(
                title="User could not be verified. Ticket is CLOSED.", user=self.request.user, ticket=ticket
            )
            fup.save()
            req.approval_requested = False
            req.complete = True
        ...
        if role:
            ...
            if (
                UserAssignmentWeight.objects.filter(role=role, user=self.request.user).exists()
                ...
                if self.request.user == req.initiated_by:
                    if not (self.request.user.is_superuser):
                        return JsonResponse(
                            {
                                "error": "This process requires 2-person validation. Since you are the user that initiated this process, you cannot authorize it."
                            },
                            status=401,
                        )
                ...
                if req.contact.active == False:
                    ...
                    _add_activity(
                        ...
                        3,
                        req.contact,
                        "changed status to ACTIVE through contact association process",
                    )

                # then approve
                ec, created = EmailContact.objects.update_or_create(
                    contact=req.contact,
                    email=req.user,
                ...
                VinceCommEmail.objects.update_or_create(
                    contact=vccontact, email=req.user, defaults={"email_list": False, "name": full_name}
                )
                _add_activity(
                    self.request.user, 3, req.contact, f"added email {req.user} through contact association tool"
                )
                if vc_user:
                    ...
                    if ga == None:
                        # add group admin
                        ga = GroupAdmin.objects.update_or_create(contact=req.contact, email=ec)
            ...
            return JsonResponse(
                {"error": "You are not an authorizer. You are not permitted to perform this action."}, status=401
            )
...
class ContactRequestAuth(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("remove"):
            ...
            fup = FollowUp(title="User could not be verified", user=self.request.user, ticket=ticket)
            fup.save()
            req.complete = True
            req.save()
            return JsonResponse({"status": "success"}, status=200)
...
class MessageAdminAddUser(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if num_admins:
            ...
            if tmpl:
                ...
                context["msg_body"] = tmpl.plain_text.replace("[VENDOR]", contact.vendor_name).replace(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if users:
            ...
            ticket = Ticket(
                title="VINCE Vendor User Request",
                created=timezone.now(),
                status=Ticket.CLOSED_STATUS,
                queue=vq,
            ...
            ticket.save()
            fup = FollowUp(
                title=f"Sent message to {contact.vendor_name} admins: {admins}",
                comment=self.request.POST.get("msg"),
                ticket=ticket,
                user=self.request.user,
            )
            fup.save()
        ...
        messages.success(self.request, f"Message sent successfully. Refer to ticket {ticket.ticket_for_url}.")
            ...
            if contact:
                ...
                if num_admins:
                    ...
                    if tmpl:
                        team_sig = get_team_sig(self.request.user)
                        msg_body = (
                            tmpl.plain_text.replace("[EMAIL]", self.request.POST.get("email"))
                            .replace("[VENDOR]", contact.vendor_name)
                            .replace("[team_signature]", team_sig)
                        )
                    else:
                ...
                return JsonResponse(
                    {
                        ...
                        "msg_link": msg_link,
                        "contact_link": reverse("vince:contact", args=[contact.id]),
                        "email_link": reverse("vince:initcontactverify", args=[contact.id]) + "?tkt=" + str(ticket.id),
                        "msg_body": msg_body,
                    },
                    status=200,
                )
...
class ContactVerifyInit(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("pk"):
            ...
            emails = [admin.email.email for admin in GroupAdmin.objects.filter(contact=contact.id)]
            allemails = contact.emailcontact_set.all()
            for email in allemails:
                if email.email_list:
            ...
            if self.request.GET.get("bypass"):
                ...
                messages.warning(
                    self.request,
                    _(
                        f"You have requested internal verification for this user. Please provide justification in the email field."
                    ),
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if EmailContact.objects.filter(
                contact=contact, email=self.request.POST["user"], email_list=False
            ).exists():
                form._errors.setdefault("user", ErrorList(["Email already exists for this contact"]))
                messages.error(self.request, _(f"Email already exists for this contact"))
                return render(request, self.template_name, {"form": form, "emails": contact.get_emails()})

            if VinceCommEmail.objects.filter(
                contact__vendor_id=contact.id, email=self.request.POST["user"], email_list=False
            ).exists():
                form._errors.setdefault(
                    "user",
                    ErrorList(
                        ["Email already exists for this contact in VINCEComm. Check Contact Updates and Approve"]
                    ),
                )
                messages.error(self.request, _(f"Email already exists for this contact in VINCEComm."))
                return render(request, self.template_name, {"form": form, "emails": contact.get_emails()})
            ...
            for email in emails:
                email = email.strip()
                if EmailContact.objects.filter(contact=contact, email__iexact=email).exists():
                    continue
                else:
                    if self.request.POST.get("internal"):
                        continue
                    form._errors.setdefault("email", ErrorList([f"Invalid email ({email}) for this contact."]))
                    messages.error(
                        self.request,
                        _(
                            f"Invalid verification email for this contact. Verification emails must be added to contact before initiating process."
                        ),
                    )
                    return render(request, self.template_name, {"form": form, "emails": contact.get_emails()})
    ...
    def form_invalid(self, form):
        ...
        return render(self.request, self.template_name, {"form": form, "ca": ca})
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["internal"] == True:
            authorizer_role = UserRole.objects.filter(role="Authorizer").first()
            if authorizer_role:
                authorizer = auto_assignment(authorizer_role.id, exclude=self.request.user)
            else:
            ...
            ticket.assigned_to = ticket_assignee
            ticket.save()
        ...
        # Create followup and send email notifications
        if form.cleaned_data["internal"] == False:
            ...
            fup = FollowUp(ticket=ticket, title=title, comment=body, user=self.request.user)
            fup.save()
        ...
        if form.cleaned_data["internal"] == True:
            ...
            fakerequest.POST = {
                "owner": ticket_assignee.id,
                "new_status": Ticket.OPEN_STATUS,
                "comment": "Requesting Authorization for Contact Association",
                "auto": 1,
            }
            update_ticket(fakerequest, ticket.id)
...
class ContactsSearchView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        res = chain(ca, ga)
        qs = sorted(res, key=lambda x: normalize_time(x, "action_ts"), reverse=True)
...
class ContactsResults(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_query:
            if facet == "Contacts":
                vince_results = VinceProfile.objects.using("vincecomm").filter(
                    Q(user__first_name__icontains=search_term)
                    | Q(user__last_name__icontains=search_term)
                    | Q(preferred_username__icontains=search_term)
                    | Q(user__email__icontains=search_term)
                ...
                email_results = EmailContact.objects.filter(
                    Q(email__icontains=search_term) | Q(name__icontains=search_term)
                ).values_list("contact", flat=True)
                emails = Contact.objects.filter(id__in=email_results)
        ...
        if int(sort_type) == 1:
            ...
            qs = sorted(results, key=lambda instance: instance.name, reverse=False)
...
class CreateGroupView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    def form_valid(self, form):
        ...
        for group in contactlist:
            ...
            if contact:
                newmember = GroupMember(
                    group=newgroup, contact=contact, user_added=self.request.user, member_type=contact.vendor_type
                )
                newmember.save()
                if newmember.id:
                    added = added + 1
                ...
                if contact:
                    ...
                    newmember = GroupMember(
                        group=newgroup, group_member=groupdup, user_added=self.request.user, member_type="Group"
                    )
                    newmember.save()
                    if newmember.id:
        ...
        update_srmail_file()
        messages.warning(self.request, _(f"Updating SRMAIL file due to new group."))
...
class AddContactToGroupView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("rm"):
            ...
            newmember = GroupMember(
                group=group, contact=contact, user_added=self.request.user, member_type=contact.vendor_type
            )
            newmember.save()
...
class RemoveContactView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if vinny_group:
            vinny_group.delete()
        messages.success(self.request, _("Contact successfully removed."))
...
class RemoveGroupView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        contact.delete()
        messages.success(self.request, _("Group successfully removed."))
    ...
    def form_valid(self, form):
        ...
        if vendor_type == "Group":
            ...
            if old_contact:
                ...
                return JsonResponse({"error": error_str, "oldid": old_contact.id}, status=401)
...
class GroupDetailView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["members"] = (
            GroupMember.objects.filter(group=group).exclude(contact__isnull=True).values_list("contact", flat=True)
        )
        context["group_members"] = (
            GroupMember.objects.filter(group=group).exclude(contact__isnull=True).order_by("contact")
        )
...
class GroupEditView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["contactpage"] = 1
        context["members"] = (
            GroupMember.objects.filter(group=group[0]).exclude(contact__isnull=True).values_list("contact", flat=True)
        )
        context["group_members"] = (
            GroupMember.objects.filter(group=group[0]).exclude(contact__isnull=True).order_by("contact")
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.request.GET.get("activate"):
            ...
            if members:
                ...
                messages.error(self.request, _(f"You cannot activate a group without members."))
    ...
    def form_valid(self, form):
        ...
        if current_group.version != int(self.request.POST["version"]):
            error_str = (
                "Someone beat you to editing this group. View the most recent details and retry editing this group."
            )
            group = ContactGroup.objects.filter(id=self.kwargs["pk"])
            members = (
                GroupMember.objects.filter(group=group[0])
        ...
        members = (
            GroupMember.objects.filter(group=self.kwargs["pk"])
            .exclude(contact__isnull=True)
            .values_list("contact", flat=True)
        )
        ...
        for group in contactlist:
            ...
            if contact:
                newmember = GroupMember(
                    group=current_group, contact=contact, user_added=self.request.user, member_type=contact.vendor_type
                )
                newmember.save()
                if newmember.id:
                    added = added + 1
                ...
                if contact:
                    groupdup = GroupDuplicate.objects.filter(group=contact).first()
                    newmember = GroupMember(
                        group=current_group, group_member=groupdup, user_added=self.request.user, member_type="Group"
                    )
                    newmember.save()
                    if newmember.id:
            ...
            logger.debug(f"Added new members {newmember} to Group")
        ...
        if added or removed:
            update_srmail_file()
            messages.warning(self.request, _(f"Updating SRMAIL file due to group membership changes."))
...
class ApproveContactInfoChangeView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vcphones = set(
            VinceCommPhone.objects.filter(contact=context["vccontact"]).order_by("version").values_list(*fields)
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if emaildiff:
            ...
            try:
                ...
                messages.error(self.request, _(f"Error saving Email information"))
                error = True
            ...
            if groupadmin:
                for ga in groupadmin:
                    ...
                    if ec:
                        ...
                        current = VinceCommGroupAdmin.objects.filter(
                            email=vinny_email_contact, contact=vinny_email_contact
                        ).first()
                        if current:
                            current.delete()
                        ...
                        messages.warning(
                            self.request, _(f"The email {ga} has been removed. Please reassign a new group admin.")
                        )
        ...
        if phonediff:
            ...
            try:
                ...
                messages.error(self.request, _(f"Error saving Phone information"))
                error = True
        ...
        if webdiff:
            ...
            for site in vcsites:
                site_lists.append(
                    Website(contact=contact, url=site.url, description=site.description, user_added=self.request.user)
                )
            try:
                with transaction.atomic():
                ...
                messages.error(self.request, _(f"Error saving Website information"))
                error = True
        ...
        if pgpdiff:
            ...
            for pgp in vcpgp:
                ...
                comment = (
                    ContactPgP.objects.filter(contact=contact).values_list("pgp_fingerprint", flat=True).order_by("id")
                )
                if comment:
                    comment = comment[0]
                else:
            ...
            try:
                ...
                messages.error(self.request, _(f"Error saving PGP information"))
                error = True
        ...
        if postaldiff:
            ...
            try:
                ...
                messages.error(self.request, _(f"Error saving Postal Information"))
                error = True

        if emaildiff or pgpdiff:
            update_srmail_file()
            messages.warning(self.request, _(f"Updating SRMAIL file due to changes in contact information."))
...
class RejectChangeView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if webdiff:
            ...
            site_lists = []
            for site in vcsites:
                site_lists.append(VinceCommWebsite(contact=vccontact, url=site.url, description=site.description))
            try:
                with transaction.atomic():
...
class RemoveEmailFromContact(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if all_emails == 0:
            ...
            contact.save()
            messages.warning(self.request, _(f"Deactivating contact due to removal of all email addresses."))
...
class AddEmailToContact(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if prev_email:
            return JsonResponse(
                {"text": "This user is already in the process of being associated with this vendor."}, status=200
            )

        if contact.vendor_type == "Contact":
            # notification only
            email = EmailContact(
                email=email, email_list=False, name=self.request.POST.get("name"), email_function="TO", contact=contact
            )
            email.save()

            vc_email = VinceCommEmail(
                contact=vinny_contact,
                email=email,
                email_function="TO",
                email_list=False,
            ...
            messages.success(self.request, _(f"Email address added to contact."))
        ...
        if email_type == "User":
            ...
            if self.request.POST.get("msg"):
                ...
                if users:
                    ...
                    msg = Message.new_message(
                        sender, users, None, "VINCE Vendor User Request", self.request.POST["msg"]
                    )
                    user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
                    if user_groups:
                        msg.thread.from_group = user_groups[0].groupsettings.contact.vendor_name
                    ...
                    assoc.save()
                    ticket = Ticket(
                        title=f"Contact Verification Process Initiated for {email}",
            ...
            if num_admins:
                ...
                return JsonResponse(
                    {
                        ...
                        "action_link": action_link,
                        "contact_link": reverse("vince:contact", args=[contact.id]),
                        "email_link": reverse("vince:initcontactverify", args=[contact.id])
                ...
                if emails:
                    messages.warning(
                        self.request,
                        _(
                            f"This contact does not have a group admin. Initiate contact verification process to add user."
                        ),
                    )

                    return JsonResponse(
                        {"ticket": reverse("vince:initcontactverify", args=[contact.id]) + "?email=" + email},
                        status=200,
                    )
                else:
                    return JsonResponse(
                        {
                            "text": "In order to add a user, you must add an email address to contact to verify this user works for this organization. Try adding a notification-only email address before attempting to verify a user.",
                            "bypass": reverse("vince:initcontactverify", args=[contact.id])
                            + f"?email={self.request.POST.get('email')}&bypass=1",
                        },
                        status=200,
                    )
            ...
            # notification only
            email = EmailContact(
                email=email, email_list=True, name="Notification-Only", email_function="TO", contact=contact
            )
            email.save()

            vc_email = VinceCommEmail(
                contact=vinny_contact, email=email, email_function="TO", email_list=True, name="Notification Email"
            )
            vc_email.save()

            _add_activity(self.request.user, 3, contact, f"added notification-only email: {email}")

            messages.success(self.request, _(f"Notification-only email address added to contact."))
...
class AddProductToContact(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        product = VendorProduct(name=name, sector=sectors, organization=contact)
        product.save()

        messages.success(self.request, _(f"Product {name} added to vendor."))
...
class ContactDetailView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["cases"] = VulnerableVendor.objects.filter(contact=self.kwargs["pk"], case__lotus_notes=True).order_by(
            "-date_modified"
        )[:20]
        ...
        context["assignable_users"] = EmailContact.objects.filter(
            contact=self.kwargs["pk"], email_list=False, status=True
        )
        context["contacttags"] = [tag.tag for tag in ContactTag.objects.filter(contact__id=self.kwargs["pk"])]
        context["othertags"] = [
            tag.tag
            for tag in TagManager.objects.filter(tag_type=2)
            .exclude(tag__in=context["contacttags"])
            .order_by("tag")
            .distinct("tag")
        ]
        emails = EmailContact.objects.filter(contact__id=self.kwargs["pk"]).values_list("email", flat=True)
        context["associations"] = ContactAssociation.objects.filter(
            contact__id=self.kwargs["pk"], complete=False
        ).exclude(user__in=emails)
        context["vinceusers"] = User.objects.filter(is_active=True, groups__name="vince")
        context["all_groups"] = [
            group.name for group in ContactGroup.objects.all().exclude(name__in=context["groups"])
        ]
        ...
        if vc_contact:
            ...
            if vc_contact.vendor_type != "Contact" and gc:
                ...
                context["vince_users"] = list(
                    User.objects.using("vincecomm").filter(groups=gc.group).values_list("username", flat=True)
                )
            elif vc_contact.vendor_type in ["Contact", "User"]:
                # this is prob a contact
                context["vince_users"] = list(
                    User.objects.using("vincecomm").filter(email__in=list(emails)).values_list("username", flat=True)
                )
            ...
            qs = sorted(allchanges, key=lambda x: normalize_time(x, "action_ts"), reverse=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("del_admin"):
            ...
            if ec:
                ...
                return JsonResponse(
                    {
                        "error": "This user is not listed in the Emails above.  Please add user's email to this contact."
                    },
                    status=500,
                )
        elif request.POST.get("add_tag"):
            ...
            if len(new_tag) < 50:
                if TagManager.objects.filter(tag=new_tag, tag_type=2).exists():
                    tag, created = ContactTag.objects.update_or_create(
                        contact=contact, tag=new_tag, defaults={"user": self.request.user}
                    )
                    if created:
...
class EditContact(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    PostalFormSet = inlineformset_factory(
        Contact, PostalAddress, form=PostalForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    PhoneFormSet = inlineformset_factory(
        Contact, PhoneContact, form=PhoneForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    WebFormSet = inlineformset_factory(
        Contact, Website, form=WebsiteForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    PgPFormSet = inlineformset_factory(
        Contact, ContactPgP, form=ContactPgPForm, max_num=10, min_num=1, can_delete=True, extra=0
    )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.request.GET.get("activate"):
            ...
            if emails:
                ...
                messages.error(self.request, _(f"You cannot activate a contact without email addresses."))
            ...
            if vince_users:
                messages.warning(
                    self.request,
                    _(
                        f"You are deactivating a contact with active users. Remove users from this contact if they should not access cases associated with this contact."
                    ),
                )
    ...
    def form_valid(self, form):
        ...
        if (
            self.request.POST.get("version")
            and self.request.POST.get("version").isdigit()
            and contact.version != int(self.request.POST["version"])
        ):
            error_str = "Someone beat you to editing this contact. View the most recent details and retry editing this contact."
            forms = {
                "form": self.form_class(initial=Contact.objects.filter(id=self.kwargs["pk"]).values()[0]),
                "collision": error_str,
                ...
                if old:
                    ...
                    if cd["DELETE"]:
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"removed address {cd['street']} {cd['city']} {cd['state']} {cd['zip_code']}",
                        )
                        old.delete()
                        continue
                    else:
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"modified address from {old.street} {old.city}, {old.state} {old.zip_code} {old.country} to {cd['street']} {cd['city']} {cd['state']} {cd['zip_code']} {cd['country']}",
                        )
                else:
                    _add_activity(
                        self.request.user,
                        3,
                        contact,
                        f"added address {cd['street']} {cd['city']} {cd['state']} {cd['zip_code']}",
                    )
                ...
                if old:
                    ...
                    if cd["DELETE"]:
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"removed phone {cd['country_code']} {cd['phone']} {cd['comment']}",
                        )
                        old.delete()
                        continue
                    else:
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"modified phone from {old.country_code} {old.phone} {old.phone_type} {old.comment} to {cd['country_code']} {cd['phone']} {cd['phone_type']} {cd['comment']} ",
                        )
                else:
                    _add_activity(
                        self.request.user,
                        3,
                        contact,
                        f"added phone number {cd['country_code']} {cd['phone']} {cd['comment']}",
                    )
                f.save()
                vinny_phones.append(
                    VinceCommPhone(
        ...
        for f in webformset:
            if f.is_valid():
                ...
                old = cd.get("id")
                if old:
                    nochanges = all([old.url == cd["url"], old.description == cd["description"]])
                    if nochanges and not (cd["DELETE"]):
                        vinny_webs.append(
                            VinceCommWebsite(contact=vinny_contact, url=cd["url"], description=cd["description"])
                        )
                        continue
                    if cd["DELETE"]:
                        ...
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"modified web site from {old.url} {old.description} to {cd['url']} {cd['description']}",
                        )
                else:
                ...
                if old:
                    ...
                    if cd["DELETE"]:
                        ...
                        if cd["pgp_key_data"]:
                            ...
                            if cd == None:
                                messages.error(self.request, f"There was an error in parsing the PGP Key")
                                vinny_pgp.append(
                                    VinceCommPgP(
                                        contact=vinny_contact,
                        ...
                        _add_activity(
                            ...
                            3,
                            contact,
                            f"modified pgp key {old.pgp_key_id} Email: old.pgp_email {old.startdate} - {old.enddate} Revoked: {old.revoked} to {cd['pgp_key_id']} Email: cd['pgp_email'] {cd['startdate']}-{cd['enddate']} Revoked: {cd['revoked']}",
                        )
                else:
                    if cd["pgp_key_data"]:
                        cd = extract_pgp_info(cd)
        ...
        if contact.vendor_type != self.request.POST["vtype"]:
            ...
            _add_activity(
                ...
                3,
                contact,
                f"modified contact type from {contact.vendor_type} to {self.request.POST['vtype']}",
            )
        ...
        if contact.vendor_name != vendor_name:
            ...
            _add_activity(
                self.request.user, 3, contact, f"modified contact name from {contact.vendor_name} to {vendor_name}"
            )
            some_changes = True
        ...
        if contact.comment != newcomment:
            if not ((contact.comment == None) and (newcomment == "")):
                if newcomment == "":
                    ...
                    _add_activity(
                        self.request.user, 3, contact, f"modified comment from {contact.comment} to {newcomment}"
                    )
                else:
        ...
        if pgp_updates:  # or email_changes
            # rebuild srmail file
            if pgp_updates:
                messages.info(self.request, _(f"SRMAIL file has been recreated"))
            update_srmail_file()
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"].fields["vtype"].choices = [
            ("User", "User"),
            ("Vendor", "Vendor"),
...
class EditProduct(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(self.request, _(f"Product {name} updated."))
...
class EmailTemplateMgmtView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owners"] = [
            template.user.usersettings.preferred_username
            for template in context["templates"].exclude(user__isnull=True).order_by("user").distinct("user")
        ]
        return context
...
class CaseTemplateMgmtView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owners"] = [
            template.user.usersettings.preferred_username
            for template in context["templates"].order_by("user").distinct("user")
        ]
        return context
...
class EmailTemplateFilterView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST["keyword"] != "":
            ...
            res = res.filter(
                Q(plain_text__icontains=wordSearch)
                | Q(template_name__icontains=wordSearch)
                | Q(subject__icontains=wordSearch)
            )
...
class CaseTemplateFilterView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST["keyword"] != "":
            wordSearch = self.request.POST["keyword"]
            res = res.filter(
                Q(description__icontains=wordSearch)
                | Q(title__icontains=wordSearch)
                | Q(vendor_email__icontains=wordSearch)
                | Q(participant_email__icontains=wordSearch)
            )
...
class AddCaseTemplateTask(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView, FormMixin):
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"template": get_object_or_404(CaseTemplate, id=self.kwargs["pk"])})
        return kwargs
...
class NewCaseTemplate(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        readable_queues = TicketQueue.objects.filter(
            queuepermissions__group__in=self.request.user.groups.all(), queuepermissions__group_write=True
        ).distinct()
        form.fields["queue"].choices = [(q.id, q.title) for q in readable_queues]
...
class ShareArtifactView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if artifact.get_related_attachment():
            ...
            # delete the old one
            s3.Object(
                settings.VINCE_SHARED_BUCKET, settings.AWS_PRIVATE_MEDIA_LOCATION + "/" + str(vtattach.uuid)
            ).delete()
            ...
            attach = VinceTrackAttachment(file=att, vulnote=vulnote, case=vc_case)
            ...
            vf = VinceFile(
                user=self.request.user, case=case, filename=att.filename, vulnote=vulnote, comm_id=attach.id
            )
            vf.save()
            ...
            return JsonResponse(
                {
                    ...
                    "remove_link": reverse("vince:unattachfile", args=[vf.id]),
                },
                status=200,
            )
...
class VulNoteChangeRevision(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.RedirectView):
    ...
    def change_revision(self):
        revision = get_object_or_404(VulNoteRevision, id=self.kwargs["revision_id"])
...
class CloneVulnerability(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        forms = {
            "prod_formset": self.CVEProductFormSet(prefix="product", initial=products.values()),
            "cwe_formset": self.CWEFormSet(prefix="cwe", initial=cve_cwe),
            "ref_formset": self.CVEReferenceFormSet(prefix="ref", initial=cve_refs),
        }

        if cveallocator:
            context["vendor_unknown_list"] = VulnerableVendor.objects.filter(
                case=cveallocator.vul.case, deleted=False
            ).order_by("contact__vendor_name")
            for form in forms["prod_formset"]:
                form.fields["organization"].choices = [(None, "------")] + [
                    (int(u.contact.id), u.contact) for u in context["vendor_unknown_list"]
                ]
        ...
        context["allowed_tags"] = [
            tag.tag
            for tag in TagManager.objects.filter(tag_type=4)
            .filter(Q(team__in=user_groups) | Q(team__isnull=True))
            .order_by("tag")
            .distinct("tag")
        ]
...
class ShareExploitView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if exploit.share:
            ...
            vcexp = CaseVulExploit.objects.update_or_create(
                vince_id=exploit.id,
                defaults={
                    "vul": vcvul,
...
class EditVul(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        for tag in tags:
            if tag in oldtags:
                ...
                tag = VulnerabilityTag(vulnerability=vul, tag=tag, user=self.request.user)
                newt = True
                tag.save()
        # remove
        for tag in oldtags:
            if tag not in tags:
                otag = VulnerabilityTag.objects.filter(vulnerability=vul, tag=tag)
                otag.delete()
                rmt = True
        ...
        if cve:
            ...
            cve = CVEAllocation(vul=vul, cve_name=cve_name, description=vul.description, assigner=assigner)
            if refs:
                cve.references = json.dumps(refs)
            if cwes:
                cve.cwe = json.dumps(cwes)
        ...
        if old_cve != form.cleaned_data["cve"]:
            if old_cve:
                ...
                if cve_res:
                    ...
                    cve_res.save()
                    messages.warning(
                        self.request, f"Removing link to CVE Reservation {cve_res.cve_id} due to change in CVE"
                    )
        ...
        if errors:
            errors = ", ".join(errors)
            messages.error(self.request, f"Error processing vulnerability information: {errors}")

        if self.request.META.get("HTTP_REFERER") and is_safe_url(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("add_tag"):
            ...
            if len(tag) < 50:
                if (
                    TagManager.objects.filter(tag=tag, tag_type=4)
                    .filter(Q(team__in=user_groups) | Q(team__isnull=True))
                    .exists()
                ):
                    tag, created = VulnerabilityTag.objects.update_or_create(
                        vulnerability=vul, tag=tag, defaults={"user": self.request.user}
                    )
                    if created:
                        fup = CaseAction(
                            title=f'Vulnerability {vul.vul} tagged as "{tag}"', case=vul.case, user=self.request.user
                        )
                        fup.save()
                else:
                    logger.debug(f"invalid tag - tag {tag} doesn't exist in tag manager")
                ...
                return JsonResponse(
                    {"tag": tag, "vul": vul.id, "error": "Tag is too long. Max 50 characters."}, status=401
                )
            ...
            try:
                ...
                fup = CaseAction(
                    title=f'Removed vulnerability tag "{tag}" from {vul.vul}', case=vul.case, user=self.request.user
                )
                fup.save()
...
class EditCVEView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["vendor_unknown_list"] = VulnerableVendor.objects.filter(case=cve.vul.case, deleted=False).order_by(
            "contact__vendor_name"
        )
        cve_refs = []
        # cve_works = []
        cve_cwe = []
    ...
    def form_valid(self, form):
        ...
        if cveprodformset.is_valid():
            ...
            for f in cveprodformset:
                ...
                if cd["organization"]:
                    ...
                    checkprod = VendorProduct.objects.filter(
                        name__iexact=cd["cve_affected_product"], organization=contact[0]
                    )
                    if not checkprod:
                        logger.info(
                            f"Creating a new Product under vendor {contact[0]} as {cd['cve_affected_product']}"
                        )
                    ...
                    if oldprods.count() == len(cveprodformset):
                        ...
                        for _prod in oldprods:
                            if (
                                (cd["version_affected"] == _prod.version_affected)
                                & (cd["version_name"] == _prod.version_name)
                                & (cd["version_value"] == _prod.version_value)
                                & (cd["cve_affected_product"] == _prod.product.name)
                                & (contact[0] == _prod.product.organization)
                                & (prodchanged == False)
                            ):  # & (cd['organization'] == _prod.product.organization)
                                if falseindx > 0:
                                    falseindx = 0
                                break
                            else:
        ...
        if (
            ...
            and (self.request.path not in self.request.META.get("HTTP_REFERER"))
            and is_safe_url(self.request.META.get("HTTP_REFERER"), set(settings.ALLOWED_HOSTS), True)
        ):
            return HttpResponseRedirect(self.request.META.get("HTTP_REFERER") + "#vuls")
        else:
...
class CVEFormView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    form_class = CVEAllocationForm
    template_name = "vince/cveform.html"
    CVEProductFormSet = formset_factory(CVEAffectedProductForm, max_num=10, min_num=1, can_delete=True, extra=0)
    CVEReferenceFormSet = formset_factory(CVEReferencesForm, max_num=10, min_num=1, can_delete=True, extra=0)
    # CVEWorkaroundFormSet = formset_factory(CVEWorkaroundForm, max_num=10, min_num=1, can_delete=True,
    #                                       extra=0)
    CWEFormSet = formset_factory(AddCVECWEForm, max_num=5, min_num=1, can_delete=True, extra=0)
        ...
        if "vul" in self.kwargs:
            ...
            if vul.case.team_owner.name == settings.ORG_NAME:
                ...
                vulnote.append(
                    {
                        "ref_source": "CERT-VN",
                        "reference": settings.KB_SERVER_NAME + reverse("vincepub:vudetail", args=[vul.case.vuid]),
                    }
                )
        form = CVEAllocationForm(initial=initial)

        forms = {
            "prod_formset": self.CVEProductFormSet(prefix="product"),
...
class AddVul(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        return redirect(reverse("vince:case", args=[case.id]) + "#vuls")
...
class ApproveVendor(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if vendor.approve_ticket:
            ...
            f = FollowUp(
                title="Statements approved and ticket closed.",
        ...
        return JsonResponse(
            {"success": True, "link": reverse("vince:case", args=[vendor.case.id]) + "#vendors"}, status=200
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not vul.approved:
            ...
            if vul.vendor.approve_ticket:
                comment = f"{self.request.user.usersettings.preferred_username} has {comment}"
                f = FollowUp(
                    title="Approved vul-specific statement.",
                    ticket=vul.vendor.approve_ticket,
                    date=timezone.now(),
                    comment=comment,
                    user=self.request.user,
                )
...
class RemoveVulSSVCView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ssvc.delete()
        messages.success(self.request, _(f"Score has been successfully removed"))
...
class VulSSVCView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        vul = get_object_or_404(Vulnerability, id=self.kwargs["pk"])
        vulssvc = VulSSVC.objects.update_or_create(
            vul=vul,
            defaults={
                "state": exploit,
...
class VulnerabilityView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["vendor_list"] = VendorStatus.objects.filter(vul=context["vul"]).order_by(
            "vendor__contact__vendor_name"
        )
        vlist = context["vendor_list"].values_list("vendor__id", flat=True)
        context["vendor_unknown_list"] = (
            VulnerableVendor.objects.filter(case=context["vul"].case, deleted=False)
            .exclude(id__in=vlist)
            .order_by("contact__vendor_name")
        )
...
class IgnoredUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["ignored_users"] = (
            User.objects.using("vincecomm")
            .filter(vinceprofile__pending=True, vinceprofile__ignored=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        for vendor in vendors:
            ...
            if contact:
                ...
                email, created = EmailContact.objects.update_or_create(
                    contact=contact,
                    email=newuser.username,
                    defaults={"name": newuser.get_full_name(), "user_added": self.request.user},
                )
                vinny_contact = VinceCommContact.objects.filter(vendor_id=contact.id).first()
                vinny_email_contact, created = VinceCommEmail.objects.update_or_create(
                    contact=vinny_contact,
                    email=newuser.username,
                    defaults={"name": newuser.get_full_name(), "email_list": False},
                )
                _add_activity(
                    self.request.user, 3, email.contact, f"approved user {newuser.username} and added to contact"
                )

                messages.success(self.request, _(f"User has been added to Contact: {vendor}"))
...
class ApproveUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["pending_users"] = (
            User.objects.using("vincecomm")
            .filter(vinceprofile__pending=True, vinceprofile__ignored=False)
            .order_by("-date_joined")
        )
        today = datetime.today()
        date_7 = date_rel_to_today(today, 7)
        ...
        context["last_week"] = (
            User.objects.using("vincecomm")
            .filter(date_joined__gte=date_7_str, vinceprofile__pending=False)
            .order_by("-date_joined")
        )
        return context
...
class InitiateMFAReset(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        mfatkt = MFAResetTicket.objects.update_or_create(user_id=self.kwargs["pk"], ticket=ticket)
        ...
        email = VinceEmail(
            ticket=ticket, notification=notification, user=self.request.user, email_type=1, to=user.email
        )
        email.save()
        ...
        messages.success(
            self.request,
            _(
                "Reset confirmation email sent. Refer to this ticket. This ticket should remain Open until the reset is complete."
            ),
        )
...
class ResetUserMFAView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            logger.debug(f"In ResetUserMFAView, user MFA settings were not found for {context['vc_user'].username}.")
            messages.error(self.request, f"User MFA settings were not found for {context['vc_user'].username}.")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.get("PreferredMfaSetting") == "SOFTWARE_TOKEN_MFA":
            ...
            try:
                ...
                messages.error(self.request, _("Error with resetting this user's TOTP"))
                error = True
            ...
            try:
                ...
                error = True
                messages.error(self.request, _("Error with resetting this user's SMS"))
        ...
        if not (error):
            ...
            messages.success(
                self.request,
                _(
                    f"User {totp} MFA was successfully reset. {user.username} will be prompted to re-associate MFA on next login"
                ),
            )
            ...
            reset_tkts = MFAResetTicket.objects.filter(
                user_id=user.id,
                ticket__status__in=[Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS, Ticket.IN_PROGRESS_STATUS],
            )
            for x in reset_tkts:
                ...
                x.ticket.save()
                fup = FollowUp(
                    title=f"{self.request.user.usersettings.preferred_username} reset user's MFA",
                    ticket=x.ticket,
                    user=self.request.user,
                )
                fup.save()
...
class PrintReportsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["newnotes"] = VulnerabilityCase.objects.filter(
            vulnote__date_published__year=year, vulnote__date_published__month=month, team_owner=context["my_team"]
        ).exclude(vulnote__date_published__isnull=True)
        context["updated"] = VulnerabilityCase.objects.filter(
            vulnote__date_last_published__year=year,
            vulnote__date_last_published__month=month,
            team_owner=context["my_team"],
        ).exclude(vulnote__date_published__isnull=True)
        context["ticket_emails"] = FollowUp.objects.filter(
            title__icontains="New Email", date__year=year, date__month=month, ticket__queue__in=my_queues
        ...
        context["case_emails_distinct"] = (
            context["case_emails"].order_by("ticket__case__id").distinct("ticket__case__id").count()
        )
...
class PrintWeeklyReportsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        my_queues = get_team_queues(context["my_team"])
        context["newnotes"] = VulnerabilityCase.objects.filter(
            vulnote__date_published__range=[weekstartdate, daterangeend], team_owner=context["my_team"]
        ).exclude(vulnote__date_published__isnull=True)
        context["updated"] = VulnerabilityCase.objects.filter(
            vulnote__date_last_published__range=[weekstartdate, daterangeend], team_owner=context["my_team"]
        ).exclude(vulnote__date_published__isnull=True)
        context["ticket_emails"] = FollowUp.objects.filter(
            title__icontains="New Email", date__range=[weekstartdate, daterangeend], ticket__queue__in=my_queues
        ).exclude(ticket__case__isnull=False)
        context["case_emails"] = FollowUp.objects.filter(
            title__icontains="New Email", date__range=[weekstartdate, daterangeend], ticket__queue__in=my_queues
        ).exclude(ticket__case__isnull=True)
        ...
        context["ticket_stats"] = (
            Ticket.objects.filter(queue__in=my_queues, created__range=[weekstartdate, daterangeend])
            .values("queue__title")
            .order_by("queue__title")
            .annotate(count=Count("queue__title"))
            .order_by("-count")
        ...
        context["total_closed"] = Ticket.objects.filter(
            queue__in=my_queues, created__range=[weekstartdate, daterangeend], status=Ticket.CLOSED_STATUS
        ).count()
        context["closed_ticket_stats"] = (
            Ticket.objects.filter(
                queue__in=my_queues, created__range=[weekstartdate, daterangeend], status=Ticket.CLOSED_STATUS
        ...
        active_cases = VulnerabilityCase.objects.filter(
            status=VulnerabilityCase.ACTIVE_STATUS, created__lt=weekstartdate, team_owner=context["my_team"]
        )
        deactive_cases = (
            CaseAction.objects.filter(
                title__icontains="changed status of case from Active to Inactive",
        ...
        context["new_users"] = (
            User.objects.using("vincecomm").filter(date_joined__range=[weekstartdate, daterangeend]).count()
        )
        context["total_users"] = User.objects.using("vincecomm").all().count()
        vendor_group_dict = {
            group.name: group.user_set.count()
...
class ReportsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]
...
class ReportsVulnotesView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_vulnotes.html"
    ...
    def get_context_data(self, **kwargs):
        context = super(ReportsVulnotesView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]

        context["newnotes"] = VulnerabilityCase.objects.filter(
            vulnote__date_published__year=context["year"],
            vulnote__date_published__month=context["month"],
            team_owner=context["my_team"],
        ).exclude(vulnote__date_published__isnull=True)
        context["updated"] = VulnerabilityCase.objects.filter(
            vulnote__date_last_published__year=context["year"],
            vulnote__date_last_published__month=context["month"],
            team_owner=context["my_team"],
        ).exclude(vulnote__date_published__isnull=True)
...
class ReportsEmailsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_emails.html"
    ...
    def get_context_data(self, **kwargs):
        context = super(ReportsEmailsView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]
        ...
        context["total_emails"] = len(context["ticket_emails"]) + len(context["case_emails"])
        if (context["month"] < datetime.now().month) and (context["year"] <= datetime.now().year):
            context["show_next"] = 1
        elif context["year"] < datetime.now().year:
            context["show_next"] = 1
...
class ReportsTicketsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ticketdata = Ticket.objects.filter(
            queue__in=my_queues, created__year=context["year"], created__month=context["month"]
        ).aggregate(total_tickets=Count("id"), total_closed=Count("id", filter=Q(status=Ticket.CLOSED_STATUS)))
        context["total_tickets"] = ticketdata["total_tickets"]
        context["total_closed"] = ticketdata["total_closed"]
        context["ticket_stats"] = (
            Ticket.objects.filter(queue__in=my_queues, created__year=context["year"], created__month=context["month"])
            .values("queue__title")
            .order_by("queue__title")
            .annotate(count=Count("queue__title"))
            .order_by("-count")
        ...
        context["closed_ticket_stats"] = (
            Ticket.objects.filter(
                queue__in=my_queues,
                created__year=context["year"],
                created__month=context["month"],
                status=Ticket.CLOSED_STATUS,
            )
            .values("close_reason")
            .order_by("close_reason")
...
class ReportsUserStatsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_userstats.html"
    ...
    def get_context_data(self, **kwargs):
        context = super(ReportsUserStatsView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]
        ...
        context["new_users"] = (
            User.objects.using("vincecomm")
            .filter(date_joined__month=context["month"], date_joined__year=context["year"])
            .count()
        )
        context["total_users"] = User.objects.using("vincecomm").all().count()
        vendor_group_dict = {
...
class ReportsCurrentTicketStatusView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_current_ticket_status.html"
    ...
    def get_context_data(self, **kwargs):
        logger.debug("ReportsCurrentTicketStatusView has been called")
        context = super(ReportsCurrentTicketStatusView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]
...
class ReportsCVEsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_cves.html"
    ...
    def get_context_data(self, **kwargs):
        context = super(ReportsCVEsView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]

        context["cves"] = Vulnerability.objects.filter(
            date_added__year=context["year"], date_added__month=context["month"], case__team_owner=context["my_team"]
        ).exclude(cve__isnull=True)
        logger.debug(f'the following cves were found: {context["cves"]}')
        context["cves_reserved"] = CVEReservation.objects.filter(
            time_reserved__year=context["year"],
            time_reserved__month=context["month"],
            account__team=context["my_team"],
        )

        return context


class ReportsTagsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    login_url = "vince:login"
    template_name = "vince/include/reports_page_elements/reports_page_tags.html"
    ...
    def get_context_data(self, **kwargs):
        context = super(ReportsTagsView, self).get_context_data(**kwargs)
        info_for_context, my_queues = prepare_info_for_report_element(self)
        for x in info_for_context:
            context[x] = info_for_context[x]

        context["tickettags"] = (
            TicketTag.objects.filter(ticket__queue__in=my_queues)
            .values("tag")
            .order_by("tag")
            .annotate(count=Count("tag"))
            .annotate(
                month_count=Count(
                    "tag", filter=Q(ticket__created__year=context["year"], ticket__created__month=context["month"])
                )
            )
            .order_by("-tag")
        )
        ...
        context["casetags"] = (
            CaseTag.objects.filter(case__team_owner=context["my_team"])
            .values("tag")
            .order_by("tag")
            .annotate(count=Count("tag"))
            .annotate(count=Count("tag"))
            .annotate(
                month_count=Count("tag", filter=Q(created__year=context["year"], created__month=context["month"]))
            )
            .order_by("-count")
        )

        context["vultags"] = (
            VulnerabilityTag.objects.filter(vulnerability__case__team_owner=context["my_team"])
            .values("tag")
            .order_by("tag")
            .annotate(count=Count("tag"))
            .annotate(count=Count("tag"))
            .annotate(
                month_count=Count(
                    "tag",
                    filter=Q(
                        vulnerability__date_added__year=context["year"],
                        vulnerability__date_added__month=context["month"],
                    ),
                )
            )
            .order_by("-count")
        ...
        logger.debug(f'in ReportsTagsView, team is {context["my_team"]} and context is {context}')
...
class UserGraphReport(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        users = (
            User.objects.using("vincecomm")
            .filter(date_joined__month=dt.month, date_joined__year=dt.year)
        ...
        vendorusers = (
            User.objects.using("vincecomm")
            .filter(groups__in=vendor_groups, date_joined__month=dt.month, date_joined__year=dt.year)
            .distinct()
            .annotate(day=TruncDay("date_joined"))
            .values("day")
            .annotate(c=Count("id"))
    ...
    def get_context_data(self, **kwargs):
        ...
        for x in monthly:
            ...
            count = (
                User.objects.using("vincecomm")
                .filter(groups__in=vendor_groups, date_joined__month=month, date_joined__year=year)
                .distinct()
                .count()
            )
            if count:
...
class TriageView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.ListView, FormMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "vince/searchresults.html",
            {"object_list": paginator.page(page), "total": res.count(), "show_params": 1},
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["triage_user"] = get_triage_users(self.request.user)
        context["contact_changes"] = (
            ContactInfoChange.objects.filter(approved=False).values("contact__vendor_name").distinct().count()
        )
        context["pending_users"] = (
            User.objects.using("vincecomm").filter(vinceprofile__pending=True, vinceprofile__ignored=False).count()
        )
        if self.kwargs.get("pk"):
        ...
        bounces = FollowUp.objects.filter(
            title__istartswith="Email Bounce Notification",
            ticket__status__in=[Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS],
        )
        context["pbounces"] = BounceEmailNotification.objects.filter(
            bounce_type=BounceEmailNotification.PERMANENT, action_taken=False
        )
...
class VinceCommUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["vc_user"]:
            context["activity"] = (
                VendorAction.objects.using("vincecomm").filter(user=context["vc_user"]).order_by("-created")[:30]
            )
            ...
            context["reset_mfa"] = MFAResetTicket.objects.filter(
                user_id=context["vc_user"].id,
                ticket__status__in=[Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS, Ticket.IN_PROGRESS_STATUS],
            ).exists()
...
class VinceCommRemoveUserView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        user = User.objects.using("vincecomm").filter(id=self.kwargs["pk"]).first()
        if user:
            vtuser = User.objects.filter(id=self.kwargs["pk"]).first()
            if vtuser:
                vtuser.is_active = False
                vtuser.save()
                groupInfo = list(vtuser.groups.all().values_list("name", flat=True))
                logger.debug(
                    f"{self.request.user} has made VinceTrack user {vtuser} inactive and removed from Groups: {groupInfo}"
                )
                VendorAction(
                    title=f"Administrator {self.request.user} made the VinceTrack account {vtuser} to be inactive and removed from Group {groupInfo}",
                    user=user,
                ).save()
            ...
            groupInfo = "[]"
            if groups:
                guuid = [x for x in groups.values_list("name", flat=True) if vinceutils.is_uuid(x)]
                groupInfo = list(VinceCommContact.objects.filter(uuid__in=guuid).values_list("vendor_name", flat=True))
                user.groups.clear()
            logger.debug(
                f"{self.request.user} has made VinceComm user {user} inactive and removed from Groups: {groupInfo}"
            )
            VendorAction(
                title=f"Administrator {self.request.user} made the VinceComm account {user} to be inactive and removed from Group {groupInfo}",
                user=user,
            ).save()
                ...
                if ga:
                    if ga.contact:
                        messages.warning(
                            self.request,
                            _(
                                f"This user has been removed as a group admin for {ga.contact.vendor_name}. Please reassign a new group admin."
                            ),
                        )
                email.delete()
            ...
            if self.request.POST.get("ticket"):
                ...
                if tkt:
                    ...
                    fup.save()
                    va = VendorAction(
                        user=user, title=f"{self.request.user} has made {user} inactive due to permanent bounce"
                    )
                    va.save()
            ...
            messages.success(
                self.request, _("The user has been removed from all groups and contacts and is now inactive")
            )
...
class TriageAddEvent(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("arg"):
            ...
            user_groups = self.request.user.groups.exclude(groupsettings__contact__isnull=True)
            assignable_users = User.objects.filter(is_active=True, groups__in=user_groups).order_by(
                User.USERNAME_FIELD
            )
            ...
            if created:
                return JsonResponse(
                    {"title": title, "id": event.id, "className": classname, "date": self.request.POST["date"]},
                    safe=False,
                    status=200,
                )
            else:
...
class TriageRemoveEvent(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("arg"):
            ...
            if (event.user == self.request.user or event.user_added == self.request.user) or (
        ...
        if "assign" in request.GET:
            if int(request.GET["assign"]) == 0:
                ...
                for u in users:
                    ...
                    u.usersettings.save()
                messages.success(self.request, _("Successfully unassigned triage user."))
            else:
                user = get_object_or_404(User, id=request.GET["assign"])
                user.usersettings.triage = True
                user.usersettings.save()

                messages.success(self.request, _("Successfully changed triage user."))
...
class VinceEncryptandSend(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for email in emails:
            ...
            rv = send_encrypted_mail(
                email, ticket.vrf_subject, report_template.render(context=model_to_dict(ticket)), attachment
            )
            if rv:
                # Error occurred
                messages.error(self.request, rv)
                return redirect("vince:cr", self.kwargs["pk"])
        messages.success(self.request, _("The email was successfully sent."))

        fup = FollowUp(ticket=ticket, title=f"Successfully forwarded email to {email.email}", user=self.request.user)
        fup.save()
        return redirect("vince:cr", self.kwargs["pk"])
        ...
        if self.request.POST.get("id"):
            ...
            if self.request.POST.get("remove"):
                if tag.team != user_groups[0]:
                    messages.error(self.request, "You are not permitted to remove another team's tags")
                    return redirect("vince:tags")
                if tag.team == None and not self.request.user.is_superuser:
                    messages.error(self.request, "You are not permitted to remove a global tag")
                    return redirect("vince:tags")
            ...
            existing = TagManager.objects.filter(
                tag=self.request.POST["tag"].lower(), tag_type=self.kwargs["pk"], team__in=user_groups
            ).first()
            if form.is_valid():
                if existing:
...
class VinceTeamsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["teams"] = Group.objects.exclude(groupsettings__contact__isnull=True)
        context["my_teams"] = self.request.user.groups.exclude(groupsettings__contact__isnull=True).values_list(
            "id", flat=True
        )
        if context["my_teams"]:
...
class VinceTeamSettingsView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if vinceutils.deepGet(context["team"].groupsettings.metadata, "reports.weekly") == "on":
            initial["weekly_report_boolean"] = context["team"].groupsettings.metadata["reports"]["weekly"]
        if vinceutils.deepGet(context["team"].groupsettings.metadata, "reports.recipients"):
            initial["weekly_report_recipients"] = (
                context["team"]
                .groupsettings.metadata["reports"]["recipients"]
                .replace('"', "")
                .replace("'", "")
                .replace("[", "")
                .replace("]", "")
        ...
        context["activity"] = Action.objects.filter(
            comment="Team Settings Change", user__groups__in=[context["team"]]
        ).order_by("-date")[:20]
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.POST.get("vulnote_template") != team.groupsettings.vulnote_template:
            ...
            # create Action
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} edited {team.name} team's vul note template",
                user=self.request.user,
        ...
        if self.request.POST.get("team_signature") != team.groupsettings.team_signature:
            ...
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} edited {team.name} team's email signature",
                user=self.request.user,
                comment="Team Settings Change",
            )
            action.save()
        ...
        if self.request.POST.get("disclosure_link") != team.groupsettings.disclosure_link:
            ...
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} edited {team.name} team's disclosure guidance link",
                user=self.request.user,
                comment="Team Settings Change",
            )
            action.save()
        ...
        if self.request.POST.get("cna_email") != team.groupsettings.cna_email:
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} changed {team.name} team's CNA email address from {team.groupsettings.cna_email} to {self.request.POST['cna_email']}",
                user=self.request.user,
                comment="Team Settings Change",
            )
            action.save()
        ...
        elif vinceutils.deepGet(team.groupsettings.metadata, "reports") == None:
            # initialize reports in metadata
            team.groupsettings.metadata["reports"] = {}

        if self.request.POST.get("weekly_report_boolean") != vinceutils.deepGet(
            team.groupsettings.metadata, "reports.weekly"
        ):
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} changed {team.name} team's weekly report setting from {vinceutils.deepGet(team.groupsettings.metadata,'reports.weekly')} to {self.request.POST.get('weekly_report_boolean')}",
                user=self.request.user,
                comment="Team Settings Change",
            )
            action.save()
        ...
        if self.request.POST.get("weekly_report_recipients") != vinceutils.deepGet(
            team.groupsettings.metadata, "reports.recipients"
        ):
            action = Action(
                title=f"{self.request.user.usersettings.preferred_username} changed {team.name} team's set of weekly report recipients from {vinceutils.deepGet(team.groupsettings.metadata,'reports.recipients')} to {self.request.POST.get('weekly_report_recipients')}",
                user=self.request.user,
                comment="Team Settings Change",
            )
            action.save()
        ...
        messages.success(self.request, _("Your changes were successfully saved."))
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("type"):
            ...
            if t == 1:
                ...
                group_user = [
                    group.groupcontact.contact.vendor_id
                    for group in Group.objects.using("vincecomm").exclude(groupcontact__isnull=True)
                ...
                context["results"] = Contact.objects.filter(active=True, id__in=group_user).exclude(
                    id__in=vcgroupadmins
                )
            elif t == 2:
                group_user = [
                    group.groupcontact.contact.vendor_id
                ...
                context["results"] = User.objects.using("vincecomm").filter(
                    vinceprofile__multifactor=False, is_active=True
                )
            elif t == 5:
                # contacts with EMAIL type emails also associated with a user
                all_users = list(
                    User.objects.using("vincecomm").filter(is_active=True).values_list("email", flat=True)
                )
                emails = EmailContact.objects.filter(
                    email__in=all_users, email_function__in=["EMAIL", "REPLYTO"]
                ).values_list("contact__id", flat=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            if form.cleaned_data["password1"] != form.cleaned_data["password2"]:
                form._errors.setdefault("password2", ErrorList(["Passwords did not match"]))
                return super().form_invalid(form)
            ...
            if response == None:
                if old_user:
                    messages.error(self.request, _(f"Error creating user: {error}. This user may already exist."))
                    form._errors.setdefault("email", ErrorList(["Check valid email address"]))
                else:
                    messages.error(self.request, _(f"Error creating user. {error}"))

                return super().form_invalid(form)

            if response.get("User"):
                if self.request.POST.get("send_email"):
                    messages.success(self.request, _("The user was successfully created and notified."))
                else:
                    messages.success(
                        self.request,
                ...
                useraction = CognitoUserAction(
                    title=f"created new user", user=self.request.user, email=form.cleaned_data["email"]
                )
                useraction.save()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "wordSearch" in self.request.POST:
            if self.request.POST["wordSearch"]:
                ...
                emails = VendorNotificationEmail.objects.extra(
                    where=["search_vector @@ (to_tsquery('english', %s))=true"], params=[wordSearch]
                ).values_list("id", flat=True)
                if emails:
                    res = res.filter(
                        Q(notification__id__in=emails)
                        | Q(pgp_key_id__icontains=self.request.POST["wordSearch"])
                        | Q(to__icontains=self.request.POST["wordSearch"])
                    )
                else:
                    res = res.filter(
                        Q(pgp_key_id__icontains=self.request.POST["wordSearch"])
...
class SendEmailAll(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _("Your email has been sent."))
...
class CreateNewEmailView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def form_valid(self, form, pgpkeys):
        ...
        if form.cleaned_data["email_type"] == 3:
            if len(emails) > 1:
                # Error occurred
                messages.error(self.request, f"VINCE only supports 1 email address per SMIME Certificate")
                return redirect("vince:newemail")
            if not (form.cleaned_data["new_certificate"]) and not (form.cleaned_data["certificate"]):
                logger.debug(form.cleaned_data)
                messages.error(self.request, f"A certificate is required for SMIME Encryption")
                return super().form_invalid(form)
                ...
                for email in emails:
                    ...
                    if pgp_key_data:
                        if len(pgp_key_data) > 1:
                            ...
                            if pgp_dict == None:
                                messages.error(self.request, f"There was an error parsing the PGP Key for { email }")
                                return redirect("vince:newemail")
                            sendmail.append(
                                {
                                    "email": email,
                                    "pgp_key_data": pgp_key_data.pgp_key_data,
                                    "pgp_key_id": pgp_key_data.pgp_key_id,
                                }
                            )
                    else:
                        if not (pgpkeys) and (form.cleaned_data["pgp_key"]):
                            ...
                            pgp_dict = extract_pgp_info(pgp_dict)
                            if pgp_dict == None:
                                messages.error(self.request, "There was an error parsing the PGP Key")
                                return redirect("vince:newemail")
                            sendmail.append(
                                {
                                    "email": form.cleaned_data["to"],
                                    "pgp_key_data": form.cleaned_data["pgp_key"],
                                    "pgp_key_id": pgp_dict["pgp_key_id"],
                                }
                            )
        ...
        notification = VendorNotificationEmail(
            subject=form.cleaned_data["subject"], email_body=form.cleaned_data["email_body"]
        )
        notification.save()
        ...
        for email in emails:
            ...
            for c in contact:
                tktc = TicketContact.objects.update_or_create(contact=c.contact, ticket=tkt)
        ...
        if form.cleaned_data["email_type"] == 1:
            ...
            followup = FollowUp(ticket=tkt, title=title, user=self.request.user, comment=comment)
            followup.save()
            ...
            for mail in sendmail:
                ...
                email = AdminPGPEmail(email=mail["email"], pgp_key_data=mail["pgp_key_data"])
                rv = send_encrypted_mail(email, subject, form.cleaned_data["email_body"])
                if rv:
                    # Error occurred
                    messages.error(self.request, f"Error sending mail to {mail['email']}: {rv}")
                    error = True
                else:
                    messages.success(self.request, f"Successfully sent mail to {mail['email']}")
                    title = (
                        f"New PGP Encrypted Email sent from VINCE to {email.email} using KEY ID: {mail['pgp_key_id']}"
                    )
                    comment = form.cleaned_data["email_body"]
                    if len(title) > 300:
                        title = title[:295] + ".."
            ...
            if form.cleaned_data["new_certificate"]:
                cert, created = VinceSMIMECertificate.objects.update_or_create(
                    email=form.cleaned_data["to"], defaults={"certificate": form.cleaned_data["new_certificate"]}
                )
            else:
                cert = form.cleaned_data["certificate"]
            ...
            try:
                ...
                error = True
                messages.error(
                    self.request, "Error occurred while trying to encrypt the email. Is certificate in .pem format?"
                )
            if rv:
                followup = FollowUp(
                    ticket=tkt,
        ...
        if self.request.POST:
            ...
            if self.kwargs.get("reply"):
                ...
                if fup.comment:
                    ...
                    match = re.search(
                        'New [E|e]mail( received)? from "?(?P<from_email>[^"]+)"? "(?P<subject>[^"]+)"( to (?P<to_email>.+?(?=CERT)))?',
                        fup.title,
                    )
                    if match:
            ...
            if self.kwargs.get("user"):
                ...
                emails = [
                    x.email.email for x in VinceCommGroupAdmin.objects.filter(contact__vendor_id=self.kwargs["pk"])
                ]
                ...
                if emails:
                    initial = {
                        "to": emails.email,
                        "pgp_key": emails.pgp_key_data,
                        "ticket": self.kwargs["tkt"],
                        "email_body": tkt.description,
                        "subject": "New VINCE Report",
                        "email_type": 2,
                    }
                    messages.warning(
                        self.request,
...
def DownloadCVEJson(request, pk):
    if is_in_group_vincetrack(request.user):
        ...
        cve_json["source"] = {"discovery": cve.source if cve.source else "UNKNOWN", "defect": [], "advisory": ""}
...
def CreateCVE5Json(request, pk):
    if is_in_group_vincetrack(request.user):
        ...
        for p in products:
            affect = {
                "vendor": p.organization,
                "product": p.name,
                "versions": [{"status": "affected", "version": p.version_value}],
            }
        ...
        if cve.cwe:
            for cwe in json.loads(cve.cwe):
                problems.append({"descriptions": [{"lang": "en", "description": cwe}]})
        if problems:
...
class ReadEmailAdminView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(self.request, "Email Processed")
        except:
            logger.warning(traceback.format_exc())
            messages.error(self.request, "Problem with processing email")
...
class UserReportView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["newnotes"] = VulnerabilityCase.objects.filter(
            id__in=my_cases, vulnote__date_published__year=year, vulnote__date_published__month=month
        ).exclude(vulnote__date_published__isnull=True)
        context["updated"] = VulnerabilityCase.objects.filter(
            id__in=my_cases, vulnote__date_last_published__year=year, vulnote__date_last_published__month=month
        ).exclude(vulnote__date_published__isnull=True)
        context["emails"] = VinceEmail.objects.filter(
            user=context["select_user"], created__year=year, created__month=month
        )
        new_cases = VulnerabilityCase.objects.filter(
            created__year=year, created__month=month, id__in=my_cases
        ).order_by("created")
        date_month = date(year, month, 1)
        active_cases = VulnerabilityCase.objects.filter(
            status=VulnerabilityCase.ACTIVE_STATUS, created__lt=date_month, id__in=my_cases
        )
        deactive_cases = (
            CaseAction.objects.filter(
                title__icontains="changed status of case from Active to Inactive",
        ...
        if (month < datetime.now().month) and (year <= datetime.now().year):
            ...
            context["show_next"] = 1
        context["total_tickets"] = Ticket.objects.filter(
            created__year=year, created__month=month, assigned_to=context["select_user"]
        ).count()
        context["ticket_stats"] = (
            Ticket.objects.filter(created__year=year, created__month=month, assigned_to=context["select_user"])
            .values("queue__title")
            .order_by("queue__title")
            .annotate(count=Count("queue__title"))
            .order_by("-count")
        ...
        ticket_changes = TicketChange.objects.filter(
            followup__ticket__assigned_to=context["select_user"],
            followup__date__year=year,
            followup__date__month=month,
            field="Status",
            new_value="Closed",
        ).values_list("followup__ticket__id", flat=True)
        context["total_closed"] = Ticket.objects.filter(id__in=ticket_changes).count()
        context["closed_ticket_stats"] = (
            Ticket.objects.filter(id__in=ticket_changes)
            .values("close_reason")
            .order_by("close_reason")
            .annotate(count=Count("close_reason"))
            .order_by("-count")
        ...
        context["tickets"] = tickets_in_queues.exclude(
            status__in=[Ticket.CLOSED_STATUS, Ticket.RESOLVED_STATUS, Ticket.DUPLICATE_STATUS]
        )
        context["ticketsjs"] = [obj.as_dict() for obj in context["tickets"]]
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if error_code in ["UserNotFoundException"]:
                messages.error(self.request, _(f"User {self.request.POST['email']} doesn't exist"))
        ...
        if data:
            ...
            initial["preferred_username"] = next(
                (item["Value"] for item in dicts if item["Name"] == "preferred_username"), None
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            if self.request.POST.get("old_email") != self.request.POST.get("email"):
                ...
                if other_user:
                    messages.error(
                        self.request, _(f'Error modifying user: {self.request.POST["email"]} already exists. ')
                    )
                    return render(request, "vince/user_search.html", {"form": UserSearchForm()})

            response, error = admin_change_user_details(request, self.request.POST.get("old_email"))
            if response == None:
                messages.error(self.request, _(f"Error modifying user: {error}. "))
                return render(request, "vince/user_search.html", {"form": UserSearchForm()})
            else:
                if change_email:
                ...
                useraction = CognitoUserAction(title=title, user=self.request.user, email=form.cleaned_data["email"])
                useraction.save()

                messages.success(self.request, _(f"User {self.request.POST['email']} successfully updated"))
                return render(request, "vince/user_search.html", {"form": UserSearchForm()})
        else:
            logger.debug(f"Form error in {self.__class__.__name__} post: {form.errors}")
            return super().form_invalid(form)
...
class ManageRoleAddUser(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["role"].group:
            assignable_users = User.objects.filter(is_active=True, groups__in=[context["role"].group]).order_by(
                User.USERNAME_FIELD
            )
        else:
            assignable_users = User.objects.filter(is_active=True, groups__name="vince").order_by(User.USERNAME_FIELD)
...
class VulReserveCVEView(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if account == None:
                messages.error(self.request, _(f"Error: No matching active CVE Services account"))
                if self.kwargs.get("pk"):
        ...
        if self.kwargs.get("pk"):
            ...
            if cve:
                ...
                cve = CVEAllocation(
                    vul=vul, cve_name="PLACEHOLDER", assigner=account.email, description=vul.description
                )
                cve.save()
        ...
        if cve_lib.ping() is None:
            try:
                ...
                for x in reserved["cve_ids"]:
                    cveres = CVEReservation(
                        cve_info=cve, cve_id=x["cve_id"], account=account, user_reserved=self.request.user
                    )
                    cveres.save()
                ...
                messages.error(self.request, _(f"Error Requesting CVE ID {e}"))
                return redirect(ret_link)

        else:
            messages.error(self.request, _(f"Error: CVE Service down. Try again later."))
            return redirect(ret_link)

        messages.success(
            self.request, _(f"You have successfully reserved {', '.join(new_cve_ids)}. You have {quota} remaining")
        )
        return redirect(ret_link)
...
class VINCEBounceManager(LoginRequiredMixin, TokenMixin, UserPassesTestMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["permanent"] = BounceEmailNotification.objects.filter(
            bounce_type=BounceEmailNotification.PERMANENT, action_taken=False
        ).order_by("-bounce_date")

        context["transient"] = BounceEmailNotification.objects.filter(
            bounce_type=BounceEmailNotification.TRANSIENT,
            ticket__status__in=[Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS, Ticket.IN_PROGRESS_STATUS],
        ).order_by("-bounce_date")

        context["operm"] = FollowUp.objects.filter(
            title__startswith="Email Bounce Notification",
            ticket__status__in=[Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS],
            comment__icontains="Permanent",
        )
        return context
