...
from .exceptions import ForceChangePasswordException, SoftwareTokenException, SMSMFAException
...
class AWSSRP(object):
    ...
    def authenticate_user(self, client=None):
        ...
        if response['ChallengeName'] == self.PASSWORD_VERIFIER_CHALLENGE:
            ...
            #elif tokens.get('ChallengeName') == self.SOFTWARE_TOKEN_MFA:
            #    raise SoftwareTokenException('MFA is enabled')
    ...
    def set_mfa_challenge(self, code, session, client=None):
        boto_client = self.client or client
        auth_params = self.get_auth_params()        
        challenge_response = {
	    'USERNAME': auth_params['USERNAME'],
        ...
        tokens = boto_client.respond_to_auth_challenge(
            ClientId=self.client_id,
            ChallengeName=self.SOFTWARE_TOKEN_MFA,
            ChallengeResponses=challenge_response,
            Session=session)
        if tokens.get('ChallengeName'):
            if tokens['ChallengeName'] == self.SOFTWARE_TOKEN_MFA:
                code_response = boto_client.respond_to_auth_challenge(
                    ClientId=self.client_id,
                    ChallengeName=self.SOFTWARE_TOKEN_MFA,
	            Session=tokens['Session'],
                    ChallengeResponses=challenge_response)
    ...
    def set_sms_challenge(self, code, session, client=None):
        boto_client = self.client or client
        auth_params = self.get_auth_params()
        challenge_response = {
            'USERNAME': auth_params['USERNAME'],
        ...
        tokens = boto_client.respond_to_auth_challenge(
            ClientId=self.client_id,
            ChallengeName=self.SMS_MFA,
            ChallengeResponses=challenge_response,
            Session=session)
        if tokens.get('ChallengeName'):
            if tokens['ChallengeName'] == self.SMS_MFA:
                code_response = boto_client.respond_to_auth_challenge(
                    ClientId=self.client_id,
                    ChallengeName=self.SOFTWARE_TOKEN_MFA,
                    Session=tokens['Session'],
