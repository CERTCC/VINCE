...
from .exceptions import TokenVerificationException, SoftwareTokenException, SMSMFAException
...
class Cognito(object):
    ...
    def __init__(
        ...
        logger.debug(f"self.user_pool_id: {self.user_pool_id} user_pool_id: {user_pool_id}")
    ...
    def authenticate(self, password):
        ...
        if tokens.get('ChallengeName') == aws.SOFTWARE_TOKEN_MFA:
            self.session = tokens.get('Session')
            challengeparams = tokens.get('ChallengeParameters')
            device_name=None
            if challengeparams:
                device_name = challengeparams.get('FRIENDLY_DEVICE_NAME')
            raise SoftwareTokenException(device_name)
        elif tokens.get('ChallengeName') == aws.SMS_MFA:
            self.session = tokens.get('Session')
            raise SMSMFAException('SMS MFA is enabled')
        self.verify_token(tokens['AuthenticationResult']['IdToken'],'id_token','id')
        self.refresh_token = tokens['AuthenticationResult']['RefreshToken']
        self.verify_token(tokens['AuthenticationResult']['AccessToken'], 'access_token','access')
        self.token_type = tokens['AuthenticationResult']['TokenType']
    ...
    def get_user(self, attr_map=None):
        ...
        user_metadata = {
            ...
            'mfa': user.get('UserMFASettingList'),
