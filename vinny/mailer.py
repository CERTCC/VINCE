...
from vince.models import BounceEmailNotification
...
def safe_template_context(message):
    ...
    if message.thread.case:
        if message.thread.case.team_owner:
            context.update({"team": team_template_context(message.thread.case.team_owner)})
...
def send_newmessage_mail(message, user, notrack=True):
    ...
    send_templated_mail(template, context, recipients=user.email, fail_silently=True, files=None, html=html)
...
def send_group_mention_notification(user, group, mentioned_by, post, tracking=None):
    ...
    if post.case.team_owner:
        tmpl_context.update({"team": team_template_context(post.case.team_owner)})
    ...
    send_templated_mail("group_tagged", tmpl_context, recipients=user.email, fail_silently=True, files=None, html=html)


def send_user_mention_notification(user, mentioned_by, post):
    """Send user mention notification to user."""
    subject = "[%s] You were tagged in a post" % (post.case.vu_vuid)
    item_url = post.case.get_absolute_url()
    tmpl_context = {
        "mentioned_by": mentioned_by,
        "post_link": f"{settings.SERVER_NAME}{item_url}",
    ...
    send_templated_mail("user_tagged", tmpl_context, recipients=user.email, fail_silently=True, files=None, html=html)
...
def send_daily_digest_mail(user, text):
    ...
    send_templated_mail("daily_digest", context, recipients=user.email, fail_silently=True, files=None, html=html)
...
def send_post_notification(post, user, tracking=None, role=0):
    ...
    send_templated_mail(template, tmpl_context, recipients=user.email, fail_silently=True, files=None, html=html)
...
def send_templated_mail(
    ...
    try:
        ...
        try:
            t = (
                EmailTemplate.objects.using("default")
                .filter(template_name__iexact=template_name, locale__isnull=True)
                .first()
            )
        except EmailTemplate.DoesNotExist:
    ...
    if not hasattr(t, "subject"):
        ...
        return  # just ignore if template doesn't exist
    subject_part = (
        from_string("VINCE %(subject)s" % {"subject": t.subject}).render(context).replace("\n", "").replace("\r", "")
    )
    ...
    if bcc:
        logger.debug(f"in VINCE Comm bcc is {bcc}")
    ...
    try:
        ...
        logger.debug("something went wrong when VINCE Comm tried to generate the list of bouncers.")

    if bouncers:
        logger.debug(f"send_templated_mail in VINCE Comm found the following bouncers: {bouncers}")

        if recipients:
            logger.debug(
                f"before VINCE Comm's attempt to remove the bouncers from recipients, the set of recipients is {recipients}"
            )
            try:
                recipients = list(set(recipients) - set(bouncers))
                logger.debug(
                    f"after VINCE Comm's attempt to remove the bouncers from recipients, the set of recipients is {recipients}"
                )
            except Exception as e:
                logger.debug(
                    f"when VINCE Comm attempted to remove bouncers from recipients, send_templated_mail encountered the following error: {e}"
                )
        else:
            logger.debug(
                f"send_templated_mail in VINCE Comm found no recipients from which to remove these bouncers: {bouncers}"
            )

        if bcc:
            logger.debug(f"before VINCE Comm's attempt to remove the bouncers from bcc, the set of bcc is {bcc}")
            try:
                bcc = list(set(bcc) - set(bouncers))
                logger.debug(f"after VINCE Comm's attempt to remove the bouncers from bcc, the set of bcc is {bcc}")
            except Exception as e:
                logger.debug(
                    f"when VINCE Comm attempted to remove bouncers from bcc, send_templated_mail encountered the following error: {e}"
                )
        else:
            logger.debug(
                f"send_templated_mail in VINCE Comm found no bcc from which to remove these bouncers: {bouncers}"
            )

    else:
        logger.debug("send_templated_mail in VINCE Comm found no bouncers")
    ...
    logger.debug(
        'Sending email using template {} with subject "{}" to {!r} from {}'.format(
            template_name, subject_part, recipients, sender
        )
    )
