...
class SignUpForm(UserCreationForm):
    organization = forms.CharField(max_length=200, label="Company/Affiliation", required=False)
    email = forms.CharField(max_length=254, required=True, label="Email address")
...
class PostForm(forms.Form):
    content = forms.CharField(widget=forms.Textarea(), label=_("Reply"), required=False)


class EditPostForm(forms.Form):
    post = forms.CharField(widget=forms.Textarea(), label=_("Edit Post"), required=False)
...
class SendMessageAllForm(forms.ModelForm):
    from_group = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "form-control"}), label=_("From Group:"), required=True, choices=()
    )
    ...
    def save(self, user, commit=True):
        send_comm_worker_msg_all(
            self.cleaned_data["to_group"],
            self.cleaned_data["subject"],
            self.cleaned_data["content"],
            user,
            self.cleaned_data["from_group"],
        )
...
class SendMessageUserForm(forms.ModelForm):
    ...
    case = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "form-control"}), label=_("Case"), required=False, choices=()
    )
    ...
    to_group = forms.MultipleChoiceField(
        ...
        choices=(),
        help_text=_(
            "Tag organization by name. Selecting an organization will send a message to all users in the group"
        ),
    )
    ...
    def get_group_choices(self):
        return [(0, "--------")] + [
            (q.id, q.groupcontact.contact.vendor_name) for q in Group.objects.all().exclude(groupcontact__isnull=True)
        ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.initial.get("to_user") is not None:
            self.fields["to_user"].choices = [
                (q.id, q.username) for q in get_user_model().objects.filter(pk=self.initial["to_user"])
            ]
            self.fields["to_group"].widget = forms.HiddenInput()
        elif self.initial.get("to_group") is not None:
            self.fields["to_group"].choices = [
                (q.id, q.groupcontact.contact.vendor_name)
                for q in Group.objects.filter(groupcontact__contact__vendor_id=self.initial["to_group"])
            ]
    ...
    def save(self, files=None, commit=True):
        ...
        if data["to_user"]:
            ...
            to_list_str = list(
                get_user_model().objects.filter(email__in=data["to_user"]).values_list("email", flat=True)
            )
        if data["to_group"]:
            contacts = VinceCommContact.objects.filter(vendor_name__in=data["to_group"]).values_list(
            ...
            to_list_str.extend(
                list(
                    Group.objects.filter(id__in=contacts)
        ...
        new_track_ticket(
            "Inbox",
            f"Direct Message from {teamname} ({self.user.vinceprofile.vince_username}) to {to_list_str}: '{data['subject']}'",
            msg.id,
            case,
            self.user.username,
        )
...
class SendMessageForm(forms.ModelForm):
    ...
    vendor = forms.CharField(label=_("Vendor Name"), max_length=300, required=False)
    ...
    content = forms.CharField(widget=forms.Textarea(), label=_("Content"), required=False)
            ...
            if hasattr(self, "vendor_pk"):
                ...
                if vc != None:
                    ...
                    if len(admin_emails) > 0:
                        ...
                        thread_id = UserApproveRequest.objects.filter(user=self.user, contact=vc).values_list(
                        ...
                        if thread_id:
                            ...
                            dup.error = True
                            dup.info = {"error": "Request already exists!", "thread_url": thread_url}
                            return dup
                        uar = UserApproveRequest(user=self.user, contact=vc, status=UserApproveRequest.Status.UNKNOWN)
                        uar.save()
                        try:
                            action = VendorAction(
                                title=f"UserApproveRequest made by {self.user} to join Vendor group {vc}",
                                user=self.user,
                            )
                            action.save()
...
```
                       """
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Could not add thread to a UserApproveRequest error is {e} for user {self.user} requesting to join {vc}"
                            )
                        return msg
        ...
        if why == 2 and data["case"]:
            ...
            if cr:
                crfup = CRFollowUp(cr=cr, title="sent message about report", user=self.user, comment=data["content"])
                crfup.save()
                vince_comm_send_sqs("CRUpdate", "Ticket", cr.vrf_id, self.user.username, None, "New Comment on CR")
                return msg
        elif why == 10:
            # vendor association
...
class MessageReplyForm(forms.ModelForm):
    content = forms.CharField(
        widget=forms.Textarea(attrs={"placeholder": "Type your reply here."}), label=_("Reply"), required=False
    )
    ...
    def save(self, files=None, commit=True):
        msg = Message.new_reply(self.thread, self.user, self.cleaned_data["content"])
        if files:
            for file in files:
                attachment = MessageAttachment.attach_file(msg, file)
        ...
        if msg.thread.case:
            ...
            vince_comm_send_sqs(
                "MessageReply",
                "Case",
                msg.thread.case.vuid,
                self.user.username,
                group,
                "Reply to Thread from " + self.user.username,
                "Inbox",
                msg.id,
            )
...
class UploadLogoForm(forms.ModelForm):
    logo = forms.FileField(
        required=False, label=_("Upload logo"), widget=forms.FileInput(attrs={"class": "vulupload"})
    )
...
class UploadDocumentForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        action = VendorAction(title="User uploaded document", user=self.user, case=self.case)
        action.save()
        ...
        new_attachment = VinceCommCaseAttachment(file=att, action=action)
        new_attachment.save()
...
class UploadFileForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        action = CRFollowUp(title="uploaded document", user=self.user, comment=comment, cr=self.report)
        action.save()
        ...
        new_attachment = ReportAttachment(file=att, action=action)
        new_attachment.save()
...
class StatementForm(forms.Form):
    mute_case = forms.BooleanField(
        label=_("Mute Case"),
        help_text=_(
            "Turn off post notifications for this case only. You will still receive important notifications from the coordination team or if you are tagged in the case discussion."
        ),
        required=False,
    )
    ...
    share = forms.BooleanField(
        ...
        help_text=_(
            "Checking this box will share your status and statement privately with all of the other participants in this case until the vulnerability note becomes publicly available."
        ),
        required=False,
    )
    ...
    statement = forms.CharField(
        ...
        help_text=_(
            "Provide a general statement for all vulnerabilities in this case. This statement will be published verbatim in our vulnerability note."
        ),
        required=False,
    )
    ...
    references = forms.CharField(
        ...
        help_text=_(
            "Provide references (URLs) for all vulnerabilities in this case. 1 URL per line. We will provide the list of references with your statement in our published vulnerability note."
        ),
        required=False,
    )
...
MESSAGE_STATUS_CHOICES = [(1, "Read"), (2, "Unread"), (3, "Deleted")]
...
class InboxFilterForm(forms.Form):
    keyword = forms.CharField(
        max_length=200,
        label="Keyword(s)",
        widget=forms.TextInput(attrs={"placeholder": "Search by keyword", "class": "asyncdelaysearch"}),
        required=False,
    )
...
class PreferencesForm(forms.Form):
    ...
    email_reporter_activity = forms.BooleanField(
        ...
        help_text=_(
            "Lets you know if a reporter (usually the person that reported the vulnerability) makes changes to a case you're involved in."
        ),
        required=False,
    )
...
class CaseRequestForm(forms.ModelForm):
    ...
    reporter_pgp = forms.CharField(
        widget=forms.Textarea(
            attrs={
                "rows": 4,
                "placeholder": "ASCII-armored public PGP key or URL (optional, but helps us communicate securely with you)",
            }
        ),
    ...
    coord_status = forms.MultipleChoiceField(
        ...
        label="What coordination actions have you taken so far?",
    ...
    please_explain = forms.CharField(max_length=20000, widget=forms.Textarea(attrs={}), required=False)
    ...
    other_vendors = forms.CharField(
        max_length=1000, label="Please list the vendors, one vendor per line.", required=False, widget=forms.Textarea()
    )
    ...
    user_file = forms.FileField(
        label="Upload File", help_text="You may specify one (1) related file to send us", required=False
    )
    ...
    cisa_please = forms.BooleanField(label="CISA Coordination", required=False)
