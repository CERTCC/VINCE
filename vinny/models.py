...
class VinceProfile(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, related_name="vinceprofile", on_delete=models.CASCADE)
    ...
    settings_pickled = models.TextField(
        ...
        help_text=_(
            "This is a base64-encoded representation of a "
    ...
    def _get_vendor_status(self):
        groups = self.user.groups.filter(groupcontact__contact__active=True).exclude(
            groupcontact__contact__isnull=True
        )
        if len(groups) >= 1:
    ...
    def _get_admin_status(self):
        admin = VinceCommGroupAdmin.objects.filter(
            email__email=self.user.email, contact__active=True, contact__vendor_type__in=["Coordinator", "Vendor"]
        )
        if admin:
            return True
        return False
    ...
    @cached_attribute
    def real_org(self):
        groups = self.user.groups.filter(groupcontact__contact__vendor_type="Vendor").exclude(
...
class VinceCommContact(models.Model):
    ...
    LOCATION_CHOICES = (("Domestic", "Domestic"), ("International", "International"))
    ...
    def get_list_email(self):
        email_list = (
            VinceCommEmail.objects.filter(contact=self, email_list=True, status=True, public=True)
            .exclude(name__icontains="service")
            .first()
        )
        if email_list:
...
class VinceCommEmail(models.Model):
    EMAIL_FUNCTION = (("TO", "TO"), ("CC", "CC"), ("EMAIL", "EMAIL"), ("REPLYTO", "REPLYTO"))
    EMAIL_TYPE = (("Work", "Work"), ("Other", "Other"), ("Home", "Home"), ("School", "School"))
    contact = models.ForeignKey(VinceCommContact, on_delete=models.CASCADE)
...
class VinceCommPgP(models.Model):
    ...
    version = models.IntegerField(default=0)
    public = models.BooleanField(default=False)
    pgp_email = models.EmailField(
        max_length=254, help_text=_("The email address that belongs with this PGP key"), blank=True, null=True
    )
...
class GroupContact(models.Model):
    ...
    contact = models.ForeignKey(VinceCommContact, on_delete=models.CASCADE, blank=True, null=True)

    default_access = models.BooleanField(
        default=True, help_text="If true, grant case access to users in group by default"
    )

    logo = models.FileField(storage=SharedMediaStorage(location="vince_logos"), blank=True, null=True)

    logocolor = models.CharField(max_length=10, default=random_logo_color)

    vincetrack = models.BooleanField(default=False, help_text="Is this a vincetrack group?")
...
class VinceCommGroupAdmin(models.Model):
    ...
    email = models.ForeignKey(VinceCommEmail, on_delete=models.CASCADE)

    comm_action = models.BooleanField(
        default=False, help_text="If true, group admin was created by another group admin"
    )
...
class VinceAttachment(models.Model):
    ...
    def _get_access_url(self):
        ...
        url = self.file.storage.url(
            self.file.name, parameters={"ResponseContentDisposition": f'attachment; filename="{filename}"'}, expire=10
        )
        return url
...
class VinceTrackAttachment(models.Model):
    file = models.ForeignKey(VinceAttachment, on_delete=models.CASCADE)
...
class VTCaseRequest(models.Model):
    ...
    WHY_NOT_CHOICES = [
        ("1", "I have not attempted to contact any vendors"),
    ...
    metadata = OldJSONField(
        help_text=_("Extensible, currently used to specify relevance to AI/ML systems"), blank=True, null=True
    )
    ...
    new_vuid = models.CharField(max_length=20, blank=True, null=True)

    coordinator = models.ForeignKey(
        Group, blank=True, null=True, help_text=_("The group assigned to this report."), on_delete=models.SET_NULL
    )
...
class CRFollowUp(models.Model):
    ...
    last_edit = models.DateTimeField(_("Last Modified Date"), blank=True, null=True)
...
class ReportAttachment(models.Model):
    ...
    action = models.ForeignKey(
        CRFollowUp, on_delete=models.CASCADE, verbose_name=_("Vendor Action"), blank=True, null=True
    )

    file = models.ForeignKey(VinceAttachment, on_delete=models.CASCADE, blank=True, null=True)

    attachment = models.FileField(storage=PrivateMediaStorage(), blank=True, null=True)
...
class Case(models.Model):
    ...
    title = models.CharField(max_length=500, help_text=_("A title for this case. Optional."))
    ...
    publicurl = models.CharField(
        max_length=500, blank=True, null=True, help_text=_("The URL for the public notice of a vulnerability.")
    )

    vince_id = models.IntegerField(default=0)

    cr = models.OneToOneField(VTCaseRequest, blank=True, null=True, on_delete=models.SET_NULL)
    ...
    note = models.OneToOneField("VCVUReport", on_delete=models.SET_NULL, blank=True, null=True)

    uid = models.CharField(max_length=50, default=generate_uuid)
...
class VinceCommCaseAttachment(models.Model):
    action = models.ForeignKey(
        "VendorAction", on_delete=models.CASCADE, verbose_name=_("Vendor Action"), blank=True, null=True
    )
...
class CaseVulnerability(models.Model):
    cve = models.CharField(_("CVE"), max_length=50, blank=True, null=True)
    ...
    deleted = models.BooleanField(
        default=False, help_text=_("Only True if vulnerability is removed after publication")
    )
    ...
    def _get_vul(self):
        ...
        if self.cve:
            return "CVE-%s" % self.cve
        else:
            return "%s" % self.cert_id

    vul = property(_get_vul)

    def _get_vul_for_url(self):
        """A URL-friendly vul ID, used in links."""
        return "%s-%s" % (self.case.case_for_url, self.vince_id)

    vul_for_url = property(_get_vul_for_url)

    def _get_cert_id(self):
        return "%s%s.%d" % (settings.CASE_IDENTIFIER, self.case.vuid, self.case_increment)
...
class CaseVulCVSS(models.Model):
    ...
    vul = models.ForeignKey(CaseVulnerability, on_delete=models.CASCADE)

    last_modified = models.DateTimeField(_("Last Modified Date"), default=timezone.now)

    vector = models.CharField(_("CVSS Vector String"), max_length=100, blank=True, null=True)

    score = models.DecimalField(_("CVSS Base Score"), max_digits=3, decimal_places=1, blank=True, null=True)

    severity = models.CharField(_("CVSS Severity"), max_length=20, blank=True, null=True)
...
class CaseVulExploit(models.Model):
    ...
    EXPLOIT_CHOICES = (("code", "code"), ("report", "report"), ("other", "other"))
    ...
    reference_type = models.CharField(max_length=30, choices=EXPLOIT_CHOICES, default="code")
...
class CaseMember(models.Model):
    ...
    reporter_group = models.BooleanField(default=False)
...
class CaseMemberStatus(models.Model):
    ...
    member = models.ForeignKey(CaseMember, on_delete=models.CASCADE)

    vulnerability = models.ForeignKey(CaseVulnerability, on_delete=models.CASCADE)
...
class Post(models.Model):
    ...
    group = models.ForeignKey(
        Group, blank=True, null=True, help_text=_("The group of the user"), on_delete=models.SET_NULL
    )
...
class BaseRevisionMixin(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_("user"), blank=True, null=True, on_delete=models.SET_NULL
    )
...
class PostRevision(BaseRevisionMixin, models.Model):
    ...
    content = models.TextField(blank=True, verbose_name=_("vulnote contents"))
...
class Thread(models.Model):
    ...
    to_group = models.CharField(max_length=250, blank=True, null=True)

    from_group = models.CharField(max_length=150, blank=True, null=True)
    ...
    def read(cls, user):
        return cls.objects.filter(
            userthread__user=user, userthread__deleted=False, userthread__unread=False
        ).distinct()
    ...
    def __str__(self):
        return "{}: {}".format(self.subject, ", ".join([str(user) for user in self.users.all()]))
    ...
    def attach_file(cls, message, file):
        ...
        if nopass:
            ...
            rd = s3.put_object_tagging(
                ...
                Key="vince_attachments/" + na.file.file.name,
                Tagging={
                    "TagSet": [{"Key": "Message", "Value": str(message.id)}, {"Key": "Filename", "Value": filename}]
                },
            )
...
class VendorAction(models.Model):
    ...
    created = models.DateTimeField(_("Date"), default=timezone.now)
    ...
    member = models.ForeignKey(CaseMember, on_delete=models.SET_NULL, blank=True, null=True)
...
class VendorStatusChange(models.Model):
    ...
    vul = models.ForeignKey(CaseVulnerability, on_delete=models.CASCADE, blank=True, null=True)
...
class VCVulnerabilityNote(models.Model):
    content = models.TextField(verbose_name=_("vulnote contents"))

    title = models.CharField(max_length=512, verbose_name=_("vul note title"))
    ...
    revision_number = models.IntegerField(default=1, verbose_name=_("revision number"))
...
class VCNoteVulnerability(models.Model):
    ...
    note = models.ForeignKey(VCVulnerabilityNote, related_name="notevuls", on_delete=models.CASCADE)
    ...
    def _get_cert_id(self):
        return "%s%s.%d" % (settings.CASE_IDENTIFIER, self.note.vuid, self.case_increment)
...
class VCVendor(models.Model):
    ...
    note = models.ForeignKey(VCVulnerabilityNote, related_name="vendors", on_delete=models.CASCADE)

    contact_date = models.DateTimeField(
        help_text=_("The date that this vendor was first contacted about this vulnerability."), blank=True, null=True
    )

    vendor = models.CharField(max_length=200, help_text=_("The name of the vendor that may be affected."))

    uuid = models.UUIDField(blank=True, null=True, help_text=_("The uuid of the contact."))

    references = models.TextField(help_text=_("Vendor references for this case"), blank=True, null=True)

    statement = models.TextField(
        help_text=_("A general vendor statement for all vuls in the case"), blank=True, null=True
    )
...
class VCVendorVulStatus(models.Model):
    ...
    STATUS_CHOICES = ((AFFECTED_STATUS, "Affected"), (UNAFFECTED_STATUS, "Not Affected"), (UNKNOWN_STATUS, "Unknown"))

    vendor = models.ForeignKey(VCVendor, related_name="vendorvulstatus", on_delete=models.CASCADE)

    vul = models.ForeignKey(VCNoteVulnerability, related_name="notevulnerability", on_delete=models.CASCADE)
...
class CaseTracking(models.Model):
    ...
    tracking = models.CharField(blank=True, null=True, max_length=100)
...
class CaseMemberUserAccess(models.Model):
    casemember = models.ForeignKey(CaseMember, on_delete=models.CASCADE)
