...
from vinny.models import VinceCommEmail, GroupContact, Case, Post, PostRevision, CaseMember, CaseVulnerability, CaseTracking, VinceCommGroupAdmin, CaseMemberUserAccess, VCDailyNotification, Message, VINCEEmailNotification
from django.contrib.auth.models import User, Group
import boto3
from bs4 import BeautifulSoup
from lib.vince import markdown_helpers
from vinny.mailer import send_user_mention_notification, send_group_mention_notification, send_post_notification, send_daily_digest_mail, send_newmessage_mail
import traceback
...
def update_post(data):
    ...
    if post:
        ...
        post = Post(case=case,
                    author= User.objects.filter(username=data['user']).first(),
                    pinned=True,
                    vince_id=data['pk'])
        post.save()
...
def add_vul_case(data):
    ...
    if vul:
        ...
        vul = CaseVulnerability(case=case,
                                vince_id=data['pk'],
...
def create_mail_notice(user, case=None, summary=False):
    ...
    email = VINCEEmailNotification(user=user,
                                   case=case,
                                   summary=summary)
    email.save()
        ...
        if g:
            ...
            for u in users:
                if user_in_case(u, post.case.id): # and u.vinceprofile.settings.get('email_tags'):
                    send_group_mention_notification(u, g.contact.vendor_name, post.author.vinceprofile.preferred_username, post, tracking=track_id)
                    create_mail_notice(u, post.case)
                    emails_sent.append(u.email)
    return emails_sent
...
def create_post_notification(post, user, tracking):
    ...
    obj, created = VCDailyNotification.objects.update_or_create(user=user,
                                                                case=post.case,
                                                                defaults=
                                                                {'tracking': tracking})
    if created:
        ...
        if u.participant:
            ...
            for us in u.group.user_set.all():
                ...
                if us.vinceprofile.service:
                    ...
                    if VinceCommEmail.objects.filter(email=us.email, contact=u.group.groupcontact.contact, email_function__in=['EMAIL', 'REPLYTO']).exists():
                        continue
