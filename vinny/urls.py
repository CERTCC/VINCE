...
urlpatterns = [
    ...
    re_path(
        r"attachments/(?P<type>(case|msg|report|track))/(?P<path>.*)$",
        views.VinceAttachmentView.as_view(),
        name="attachment",
    ),
    re_path(
        "limited/reports/print/(?P<month>[0-9]+)/(?P<year>[0-9]+)/$",
    ...
    re_path(
        "^vul/(?P<pk>[0-9]+)/providestmt/(?P<vendor_id>[0-9]+)/$", views.AddStatement.as_view(), name="providestmt"
    ),
    ...
    re_path(
        "profile/group_card/(?P<egid>[0-9a-fA-F]+)?/(?P<case>[0-9]+)?/$",
        views.GroupCardView.as_view(),
        name="groupcardcase",
    ),
    ...
    # Make this endpoint go live when we complete 821:
