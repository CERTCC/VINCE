...
from vince.lib import (
    ...
    create_bounce_ticket,
    send_vt_daily_digest,
    ...
    try:
        ...
        if cr.request_type == CaseRequest.GOV_FORM:
            ...
            for email in emails:
                rv = send_encrypted_mail(
                    email, cr.vrf_subject, report_template.render(context=model_to_dict(cr)), attach_object
                )
                if rv:
        ...
        if cr.metadata:
            if cr.metadata["ai_ml_system"] and cr.metadata["ai_ml_system"] == True:
                logger.debug(
                    'the code acknowledges that cr.metadata and cr.metadata["ai_ml_system"] and cr.metadata["ai_ml_system"] == True is true'
                )
    ...
    if request.method == "POST":
        ...
        if attributes.get("MessageType"):
            ...
            if attributes["MessageType"] == "UpdateStatus":
                ...
                send_vendor_email_notification(
                    [notification.vendor.contact.id],
                    notification.vendor.case,
                    notification.notification.subject,
                    notification.notification.email_body,
                )
        ...
        try:
            ...
            if data.get("affected_website"):
                ...
                data.get("ics_impact")
                and (data.get("ics_impact") == True)
                and not (
                    data.get("metadata")
                    and data["metadata"].get("ai_ml_system")
                    and data["metadata"].get("ai_ml_system") == True
                )
            ):
...
def find_cryptos(ckeys, f, srmail):
    ...
    added_list = []
    dummy_range = Range(
        start=datetime.datetime(1999, 12, 21), end=datetime.datetime(2099, 12, 21), key_id="ABC123", proto="PGP"
    )
    for key in ckeys:
        enddate = key.enddate
    ...
    while len(range_list) > 1:
            ...
            if old.start and earliest_start.start:
                ...
                if old.start == earliest_start.start:
                    ...
                    if rllen != newrllen:
                        ...
                        if old.end:
                            earliest_start = Range(
                                start=old.end + timedelta(days=1),
                                end=earliest_start.end,
                                key_id=old.key_id,
                                proto=earliest_start.proto,
                            )
        ...
        if earliest_end.end:
            f.write(
                ...
                + write_start
                + "\t"
                + earliest_end.end.strftime("%Y%m%d")
                + "\t"
                + earliest_start.proto
            )
        else:
        ...
        for r in range_list:
            ...
            if earliest_end.end and r.end:
                ...
                f.write("\tKEY=" + r.key_id)
            added_list.append(
                Range(start=earliest_start.start, end=earliest_end.end, key_id=r.key_id, proto=earliest_start.proto)
            )
        f.write("\n")
    ...
    if len(range_list) == 1:
        ...
        for old in added_list:
            ...
            # make sure not "EPOCH"
                ...
                if old.start <= crypto.start:
                    if old.end:
                        crypto = Range(
                            start=old.end + timedelta(days=1), end=crypto.end, key_id=crypto.key_id, proto=crypto.proto
                        )
        ...
        if crypto:
            ...
            if crypto.end:
                f.write(
                    srmail + "\tCRYPTO\t" + write_start + "\t" + crypto.end.strftime("%Y%m%d") + "\t" + crypto.proto
                )
            else:
...
def write_srmail(request):
    ...
    with open(tmp.name, "w") as f:
        ...
        if all_vendor_contact_group:
            ...
            not_vendors = GroupMember.objects.filter(group=all_vendor_contact_group).exclude(
                contact__vendor_type="Vendor"
            )
        ...
        all_contact = list(
            Contact.objects.filter(active=True, vendor_type="Contact")
            .exclude(emailcontact__isnull=True)
            .distinct()
            .values_list("srmail_peer", flat=True)
        )
        f.write("+_all_contacts\t" + " ".join(all_contact[0:]) + "\n")
        f.write("\n")

        # create an all-coord group:
        all_coordinators = list(
            Contact.objects.filter(active=True, vendor_type="Coordinator")
            .exclude(emailcontact__isnull=True)
            .distinct()
            .values_list("srmail_peer", flat=True)
        )
                ...
                for email in emails:
                    ...
                    if email_changed != "":
                        try:
                            ...
                            logger.warning(
                                f"Could not update email missing information {email_changed} for Contact {email} error {e}"
                            )
        ...
        s3.put_object(Body=f, Bucket=settings.ANCIENT_SRMAIL_BUCKET, Key="cmgr-contacts.srmail")
